...
def generate_recommendations_optimized(group: str = 'All', subgroup: str = 'All', 
    ...
    try:
        ...
        # Add expiry status column with smile emoji indicators
        if 'Source Expiry Days' in result.columns:
            def get_expiry_status(days):
                ...
                # Skip invalid dates (1900, blank, 0)
                if days_num == 0 or days_num > 100000 or days_num < -100000:
                    return ''
                
                # Status based on expiry days with smile emojis
                # Positive = expired (past date), 0 to -30 = expiring, < -30 = OK
                if days_num > 0:
                    return '☹️ EXPIRED'  # Sad smile - already expired
                elif days_num >= -30 and days_num <= 0:
                    return '😐 EXPIRING'  # Silent smile - expiring within 30 days
                elif days_num < -30:
                    return '😊 OK'  # Happy smile - more than 30 days until expiry
                else:
                    return ''
            
            result.insert(result.columns.get_loc('Source Expiry Days') + 1, 
                          'Expiry Status', 
                          result['Source Expiry Days'].apply(get_expiry_status))
...
def convert_to_excel(slow_df, fast_df, final_df, filters_used):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Apply conditional formatting to Source Expiry Days column (☹️ EXPIRED red, 😐 EXPIRING yellow, 😊 OK green)
        if 'Source Expiry Days' in final_df.columns:
            ...
            green_format = workbook.add_format({'bg_color': '#00FF00', 'font_color': '#000000'})
            
            # Get column index
            expiry_days_col_idx = final_df.columns.get_loc('Source Expiry Days')
            
            # Apply formatting row by row based on expiry days
            for row_idx in range(len(final_df_export)):
                days_value = final_df_export.iloc[row_idx]['Source Expiry Days']
                
                if pd.notna(days_value):
                    try:
                        days = float(days_value)
                        # Skip invalid dates (0, blank, or extreme values like 1900)
                        if days == 0 or abs(days) > 100000:
                            continue
                        
                        # Apply color based on expiry days
                        # Positive = expired (red), 0 to -30 = expiring (yellow), < -30 = OK (green)
                        if days > 0:
                            worksheet.write(row_idx + 1, expiry_days_col_idx, days, red_format)
                        elif days >= -30 and days <= 0:
                            worksheet.write(row_idx + 1, expiry_days_col_idx, days, yellow_format)
                        elif days < -30:
                            worksheet.write(row_idx + 1, expiry_days_col_idx, days, green_format)
                    except (ValueError, TypeError):
                        continue
        ...
        if not recommendations.empty:
            ...
            if 'final_recs' not in st.session_state or gen_clicked:
                ...
                preferred_cols = [
                    ...
                    'Source Expiry Date', 'Source Expiry Days', 'Expiry Status',
            ...
            # Add CSS to completely hide outer scrollbars - only show dataframe scrollbars
            st.markdown("""
                <style>
                    /* Hide ALL outer container scrollbars at every level */
                    ...
                    /* Only allow dataframe to scroll */
                    ...
                    /* Make dataframe scrollbars visible */
