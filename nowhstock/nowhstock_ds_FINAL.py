...
def main():
    ...
    with tab1:
        ...
        with col_table:
            if not slow_shops.empty:
                ...
                # Apply LIMIT to prevent 399MB crash
                display_slow = slow_shops.head(50000) if len(slow_shops) > 50000 else slow_shops
                if len(slow_shops) > 50000:
                    st.warning(f"⚠️ Showing first 50,000 of {len(slow_shops):,} slow-moving records. Apply filters (Shop/Group/SubGroup) to narrow results.")
                
                st.dataframe(display_slow, height=400, hide_index=True, column_config=col_config, use_container_width=True)
            else:
    ...
    with tab2:
        ...
        if not fast_shops.empty:
            ...
            # Apply LIMIT to prevent 399MB crash
            display_fast = fast_shops.head(50000) if len(fast_shops) > 50000 else fast_shops
            if len(fast_shops) > 50000:
                st.warning(f"⚠️ Showing first 50,000 of {len(fast_shops):,} fast-moving records. Apply filters (Shop/Group/SubGroup) to narrow results.")
            
            st.dataframe(display_fast, height=400, hide_index=True, column_config=col_config, use_container_width=True)
        else:
