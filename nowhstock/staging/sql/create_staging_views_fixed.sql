...
-- ============================================================
-- Business Logic:
-- 1. NEW: Each source keeps minimum 30 units as safety stock
-- 2. Only excess above 30 units available for transfer  
-- 3. Destination cap = MAX(dest_sales_30d, dest_wh_grn_30d_sales)
-- 4. FIXED: Window functions partition by (item_code, dest_shop) - independent allocation
-- 5. FIXED: Expiry threshold < 30 days (not < 0) - safety buffer
...
-- Source shops: Non-priority shops with transferable stock
-- ORIGINAL RULE: Must have stock > sales_30d to be a source (NOT stock > 30)
sources AS (
    SELECT 
    ...
    WHERE im.shopstock > COALESCE(im.sales_30d_wh, 0)  -- Stock > sales
      AND im.shopstock > 0
    -- STAGING: Allow ALL shops (including priority) as sources for testing
),
...
-- Calculate WH GRN +30d sales for destinations (from sales_2024)
dest_wh_grn_sales AS (
    ...
    FROM destinations d
    LEFT JOIN sales_2024 s ON TRIM(UPPER(s."ITEM_CODE")) = d.item_code
    ...
    SELECT 
        ...
        COALESCE(SUM(s."QTY"), 0) AS wh_grn_30d_sales
    FROM destinations d
    LEFT JOIN sales_2025 s ON TRIM(UPPER(s."ITEM_CODE")) = d.item_code
        AND TRIM(UPPER(s."SHOP_CODE")) = d.shop_code
        AND s."DATE_INVOICE"::date >= d.wh_grn_date::date
        AND s."DATE_INVOICE"::date <= d.wh_grn_plus_30::date
    WHERE d.wh_grn_date IS NOT NULL
...
-- Calculate destination capacity: MAX(sales_30d, wh_grn_30d_sales)
dest_capacity AS (
    SELECT 
        d.item_code,
        d.shop_code,
        ...
        GREATEST(COALESCE(wh.wh_grn_30d_sales, 0), d.sales_30d) AS cap
    FROM destinations d
    LEFT JOIN dest_wh_grn_sales_agg wh ON d.item_code = wh.item_code 
        AND d.shop_code = wh.shop_code
),

-- Combine all source-destination pairs
all_combinations AS (
    SELECT 
        s.item_code,
        s.item_name,
...
-- ALLOCATION RULE: Each destination receives qty = MAX(dest_sales_30d, dest_wh_grn_30d_sales)
-- Source available = stock - sales (original logic, fills destinations to full cap)
uncapped_recs AS (
    SELECT 
        *,
        ...
        -- Source available: stock - sales (original logic, NOT stock - 30)
        -- The 30-unit rule is just for tracking/remark, not for limiting transfers
    ...
    WHERE 
        -- ORIGINAL: Source must have stock > sales (already in sources CTE)
        ...
        -- Skip if destination overstocked and below minimum sales
        AND NOT (dest_stock > 30 AND dest_stock < LEAST(dest_sales, dest_wh_grn_30d_sales))
),

-- Apply cumulative cap with FEFO ordering
-- FIXED: Window function now partitions by (item_code, dest_shop) - independent allocation
capped_recs AS (
    SELECT 
        item_code,
        item_name,
        ...
        -- Expiry status (FIXED: 1-29 = Expiring, not 1-30)
        CASE 
            WHEN source_expiry_date IS NULL THEN 'OK'
            WHEN source_expiry_days < 0 THEN 'Expired'
            WHEN source_expiry_days BETWEEN 1 AND 29 THEN 'Expiring'  -- FIXED: 29 not 30
            WHEN source_expiry_days >= 30 THEN 'OK'
            ELSE 'OK'
        END AS expiry_status,
        
        -- CRITICAL FIX: PARTITION BY item_code, dest_shop (not just item_code)
        -- This ensures each destination gets independent allocation tracking
        SUM(
            CASE 
                WHEN source_expiry_date IS NULL THEN uncapped_qty
                WHEN source_expiry_days < 30 THEN 0  -- FIXED: Block < 30 days (not < 0)
                WHEN source_expiry_days >= 30 THEN uncapped_qty
                ELSE 0
            END
        ) OVER (
            PARTITION BY item_code, dest_shop  -- FIXED: Added dest_shop
        ...
        CASE 
            WHEN source_expiry_date IS NOT NULL AND source_expiry_days < 30 THEN 0  -- FIXED: < 30 not < 0
            ELSE LEAST(
                uncapped_qty,
                GREATEST(
                    dest_cap - COALESCE(
                        SUM(
                            CASE 
                                WHEN source_expiry_date IS NULL THEN uncapped_qty
                                WHEN source_expiry_days < 30 THEN 0  -- FIXED: < 30 not < 0
                                WHEN source_expiry_days >= 30 THEN uncapped_qty
                                ELSE 0
                            END
                        ) OVER (
...
-- Check source allocation across destinations
source_allocation_check AS (
    SELECT 
        *,
        -- Track cumulative source allocation across all destinations
        SUM(recommended_qty) OVER (
            PARTITION BY item_code, source_shop
            ORDER BY priority_rank, source_grn_age DESC, COALESCE(source_expiry_days, 999999), source_stock DESC, dest_shop
        ) AS cumulative_source_allocated
    FROM capped_recs
),

-- Final results with remarks
final_results AS (
    SELECT 
        item_code,
        item_name,
        ...
        -- Cumulative per destination (FIXED: independent tracking)
        SUM(recommended_qty) OVER (
            PARTITION BY item_code, dest_shop  -- FIXED: Added dest_shop
            ORDER BY priority_rank, source_grn_age DESC, COALESCE(source_expiry_days, 999999), source_stock DESC, source_shop
        ) AS cumulative_qty,
        ...
        -- Updated destination stock
        dest_stock + recommended_qty AS dest_updated_stock,
        
        -- Stock days after transfer
        CASE 
        ...
        -- Remark (FIXED: Expiry checks have priority over cap checks)
        CASE 
...
CREATE INDEX IF NOT EXISTS idx_mv_recs_staging_item ON mv_recommendations_complete_staging(item_code);
CREATE INDEX IF NOT EXISTS idx_mv_recs_staging_source ON mv_recommendations_complete_staging(source_shop);
CREATE INDEX IF NOT EXISTS idx_mv_recs_staging_dest ON mv_recommendations_complete_staging(dest_shop);
CREATE INDEX IF NOT EXISTS idx_mv_recs_staging_groups ON mv_recommendations_complete_staging(groups);
CREATE INDEX IF NOT EXISTS idx_mv_recs_staging_subgroup ON mv_recommendations_complete_staging(sub_group);
CREATE INDEX IF NOT EXISTS idx_mv_recs_staging_item_dest ON mv_recommendations_complete_staging(item_code, dest_shop);
...
3. Destination cap = MAX(dest_sales_30d, dest_wh_grn_30d_sales)
4. Window functions partition by (item_code, dest_shop) - independent allocation per destination
5. Expiry threshold < 30 days - items expiring within 30 days are blocked
6. FEFO ordering (First Expiry First Out - oldest stock first)
7. Cumulative allocation per destination ≤ cap
8. Priority shops (11): SPN, MSS, LFS, M03, KAS, MM1, MM2, FAR, KS7, WHL, MM3

KEY FIXES (Dec 10, 2025):
- PARTITION BY item_code, dest_shop (was missing dest_shop) - prevents overallocation
- Expiry threshold < 30 (was < 0) - 30-day safety buffer
- Expiry classification 1-29 = Expiring (was 1-30)
- Remark logic: Expiry checks before cap checks
- Source filter: stock > sales_30d (NOT stock > 30) - allows small sources to participate
- source_available = stock - 30 in uncapped_recs - safety stock logic';
