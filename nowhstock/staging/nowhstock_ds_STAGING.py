...
class Config:
    ...
    # STAGING: Use test materialized views
...
def load_slow_fast_moving_data(sales_status: str = None, shop: str = 'All', group: str = 'All', subgroup: str = 'All') -> pd.DataFrame:
    """Load slow/fast moving data from materialized view (FAST - <100ms vs 5-10s)
    
    Args:
        sales_status: 'Slow', 'Fast', or None for all
        shop: Shop code filter
        group: Group filter
        subgroup: SubGroup filter
    
    Returns:
        DataFrame with slow/fast moving inventory
    """
    try:
        where_conditions = []
        params = []
        ...
        if sales_status:
            where_conditions.append("sales_status = %s")
            params.append(sales_status)
        
        if shop and shop != 'All':
            where_conditions.append("shop_code = %s")
            params.append(shop.strip().upper())
        ...
        query = f"""
            SELECT 
                ...
                sales_30d,
                sales_status
            from mv_slow_fast_moving_summary_staging_staging
            {where_clause}
        """
        ...
        df = df.rename(columns={
            'SALES_30D': 'ITEM_SALES_30_DAYS',
            'SALES_STATUS': 'Sales_Status'
        })
        
        logger.info(f"✅ Loaded {len(df)} rows from mv_slow_fast_moving_summary_staging_staging (status={sales_status}, cached)")
        return df
        
    except Exception as e:
        logger.error(f"Error loading slow/fast moving data from view: {e}")
        logger.warning("View may not exist. Run: psql -f create_slow_fast_view_simple.sql")
        raise  # Re-raise so fallback logic can catch it
...
def load_slow_fast_moving_data(sales_status: str = None, shop: str = 'All', group: str = 'All', subgroup: str = 'All') -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f"Error loading slow/fast moving data from view: {e}")
        logger.warning("Falling back to inventory_master query...")
        # Fallback to old method if view doesn't exist
        return load_inventory_data(group, subgroup, 'All', shop)
...
def format_recommendations(df: pd.DataFrame, skip_capping: bool = False) -> pd.DataFrame:
    ...
    if skip_capping:
        ...
        if 'dest_final_stock_days' not in df.columns:
            ...
            # If both are zero, then dest_updated_stock / 30
            df['max_sales'] = df[['dest_wh_grn_30d_sales', 'dest_sales']].max(axis=1)
            df['dest_final_stock_days'] = np.where(
                df['max_sales'] > 0,
                np.ceil((df['dest_updated_stock'] / df['max_sales']) * 30),
                np.where(
                    df['dest_updated_stock'] > 0,
                    np.ceil(df['dest_updated_stock'] / 30),
                    0
                )
            )
...
def generate_recommendations_optimized(group: str = 'All', subgroup: str = 'All', 
    ...
    # If no limit specified and all filters are 'All', apply 50K default
    if limit is None:
        if group == 'All' and subgroup == 'All' and product == 'All' and shop == 'All':
            limit = 50000
            logger.warning(f"⚠️ No filters applied - using default LIMIT {limit} to prevent overload")
        else:
            # Filters applied - still cap at reasonable max
            limit = 100000
    
    logger.info(f"🚀 STAGING: Querying {Config.MV_RECOMMENDATIONS} (LIMIT: {limit})")
    logger.info(f"🔍 PARAMS: group={group}, subgroup={subgroup}, product={product}, shop={shop}")
    ...
    if group and group != 'All':
        where_conditions.append("groups = %s")
        params.append(group)
        logger.info(f"  ➕ Added group filter: {group}")
    if subgroup and subgroup != 'All':
        where_conditions.append("sub_group = %s")
        params.append(subgroup)
        logger.info(f"  ➕ Added subgroup filter: {subgroup}")
    if product and product != 'All':
        where_conditions.append("item_code = %s")
        params.append(product.strip().upper())
        logger.info(f"  ➕ Added product filter: {product}")
    if shop and shop != 'All':
        where_conditions.append("(source_shop = %s OR dest_shop = %s)")
        params.extend([shop.strip().upper(), shop.strip().upper()])
        logger.info(f"  ➕ Added shop filter: {shop} (applies to source OR dest)")
    ...
    query = f"""
        SELECT 
            ...
            remark
        FROM {Config.MV_RECOMMENDATIONS}
        {where_clause}
    ...
    try:
        ...
        logger.error(f"❌ Error querying {Config.MV_RECOMMENDATIONS}: {e}")
        st.error(f"❌ STAGING view not available.\n\nPlease run:\n1. cd d:\\Dashboard Code\\NO_WH\\DS\\nowhstock\\staging\n2. psql -U postgres -d salesdata -p 3307 -f sql\\create_staging_views.sql")
        return pd.DataFrame()
...
def generate_recommendations(df: pd.DataFrame, use_grn_logic: bool = True, threshold: int = 10) -> pd.DataFrame:
    ...
    for (item, src_shop), g in merged.groupby(['ITEM_CODE', 'SHOP_CODE_SRC'], observed=True):
        ...
        for dest_rec in destination_recommendations:
            ...
            # Formula: dest_updated_stock / MAX(dest_wh_grn_30d_sales, dest_sales) * 30
            # If both are zero, then dest_updated_stock / 30
            max_sales = max(dest_rec.get('dest_wh_grn_30d_sales', 0), dest_rec.get('dest_sales', 0))
            if max_sales > 0:
                final_stock_days = (dest_updated_stock / max_sales) * 30
            elif dest_updated_stock > 0:
                final_stock_days = dest_updated_stock / 30
            else:
                final_stock_days = 0
...
def main():
    ...
    # Load data using FAST materialized view (100x faster than inventory_df)
    logger.info(f"🚀 Loading slow/fast moving data from materialized view...")
    
    try:
        # Load from materialized view (instant - <100ms)
        all_data = load_slow_fast_moving_data(sales_status=None, shop=shop, group=group, subgroup=subgroup)
        
        if all_data.empty:
            st.warning("No records found in materialized view")
            st.stop()
        
        logger.info(f"📊 Loaded {len(all_data)} records from materialized view in <100ms")
        
        # Apply SIT filters
        all_data = apply_itemdetails_filters(all_data, sit_filter_df, item_type, supplier, item_name)
        
        # Merge TYPE information for pie chart
        if not sit_filter_df.empty:
            ...
            all_data = all_data.merge(sit_type_map, on='ITEM_CODE', how='left')
            all_data['TYPE'] = all_data['TYPE'].fillna('Unknown')
        
        # Split by sales status (already classified in view)
        slow_shops = all_data[all_data['Sales_Status'] == 'Slow'].copy()
        fast_shops = all_data[all_data['Sales_Status'] == 'Fast'].copy()
        inventory_df = all_data  # For compatibility
        
        logger.info(f"✅ Slow: {len(slow_shops)}, Fast: {len(fast_shops)}")
        
    except Exception as e:
        logger.error(f"❌ Materialized view failed: {e}, falling back to inventory_master")
        ...
        inventory_df['Sales_Status'] = np.where(
            inventory_df['ITEM_SALES_30_DAYS'] >= threshold, 'Fast', 'Slow'
        )
    ...
    if gen_clicked:
        ...
        try:
            ...
            st.info(f"🔍 Filters: Group={group}, SubGroup={subgroup}, Product={product}, Shop={shop}, Generate All={generate_all}")
            logger.info(f"🔍 FILTERS: Group={group}, SubGroup={subgroup}, Product={product}, Shop={shop}, Generate All={generate_all}")
            ...
            if generate_all:
                ...
                st.info(f"📊 Limited load: {limited_load}, Row limit: {row_limit}")
