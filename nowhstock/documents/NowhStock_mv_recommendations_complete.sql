...
sources AS (
    ...
    WHERE im.shopcode NOT IN (SELECT shop_code FROM priority_shops)  -- NOT priority shops
      AND im.shopstock > COALESCE(im.sales_30d_wh, 0)  -- ORIGINAL: Stock > sales
      AND im.shopstock > 0
),
...
uncapped_recs AS (
    SELECT 
        *,
        ...
        -- Source available: stock - 30 (FIXED: Reserve 30 units in source shop)
        -- Only excess above 30 units can be transferred to destinations
    ...
    WHERE 
        -- Source must have stock > 30 to have anything available after reserve
        source_stock > 30
...
-- Enforce source-level cap (stock - 30) across all destinations
source_capped_recs AS (
    SELECT
        *,
        CASE
            WHEN source_available <= 0 THEN 0
            ELSE GREATEST(
                LEAST(
                    recommended_qty,
                    source_available - COALESCE(
        ...
        END AS recommended_qty_source_capped
    FROM capped_recs
),
...
source_allocation_check AS (
    SELECT 
        ...
        source_available,
        expiry_status,
        recommended_qty_source_capped AS recommended_qty,
        -- Track cumulative source allocation across all destinations
        SUM(recommended_qty_source_capped) OVER (
            PARTITION BY item_code, source_shop
            ORDER BY priority_rank, source_grn_age DESC, COALESCE(source_expiry_days, 999999), source_stock DESC, dest_shop
        ) AS cumulative_source_allocated
    FROM source_capped_recs
),
