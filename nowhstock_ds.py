    ...
    try:
        ...
        query = f"""
            SELECT 
                ...
                sales_30d,
                sales_status
            FROM mv_slow_fast_moving_summary
            {where_clause}
        """
        ...
        logger.info(f"✅ Loaded {len(df)} rows from mv_slow_fast_moving_summary (status={sales_status}, cached)")
        return df
...
def generate_recommendations_optimized(group: str = 'All', subgroup: str = 'All', 
    ...
    logger.info(f"🚀 Querying mv_recommendations_complete (LIMIT: {limit})")
