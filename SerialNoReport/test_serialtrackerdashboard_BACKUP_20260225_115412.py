...
def load_wh_loading_drilldown(start_str: str, end_str: str, metric: str):
    ...
    WITH tagged AS (
        SELECT
            ...
            serial_no,
            ROW_NUMBER() OVER (
                PARTITION BY dt_doc_date::DATE, TRIM(COALESCE(serial_no,''))
                ORDER BY COALESCE(NULLIF(TRIM(wh_load_user),''),'Unknown')
            ) AS rn
        FROM serial_no_dailydata
        WHERE dt_doc_date::DATE BETWEEN ('{end_str}'::DATE - 6) AND '{end_str}'::DATE
    ),
...
def _row(label, value_str, ok, accent, bar_pct=None, note=None, stage=None, metric=None):
    ...
    # If drillable metric, navigate via query params (no JS bridge)
    if stage and metric:
        stage_q = quote(stage)
        metric_q = quote(metric)
        value_html = (
            f'<a href="?stage={stage_q}&metric={metric_q}#drill-section" target="_self" '
...
def render_drill_down_tab(stage, metric, drilldown_cache, start_s, end_s):
    ...
    def stage_has_data(stage_name: str) -> bool:
        return any(key.startswith(f"{stage_name}||") for key in drilldown_cache.keys())
...
<div style="display:flex;align-items:center;gap:12px;padding:4px 0 6px 0;">
    ...
    # ── Pre-build drilldown cache (MUST BE BEFORE INPUT PROCESSING) ────────
    drilldown_cache = {}

    # WH Receiving metrics
    for metric_wr in ["Unique %", "Unique Serials", "In-Batch Dup", "WH DB Dup", "Blank Serials", "Small (≤6)"]:
        df_wr = load_wh_receiving_drilldown(start_s, end_s, metric_wr)
        if df_wr is not None and not df_wr.empty:
            drilldown_cache[f"🏭 WH Receiving||{metric_wr}"] = df_wr
    
    # WH Loading metrics
    for metric_wl in ["Unique %", "Duplicates", "Blank Serials", "Small (≤6)"]:
        df_wl = load_wh_loading_drilldown(start_s, end_s, metric_wl)
        if df_wl is not None and not df_wl.empty:
            drilldown_cache[f"📦 WH Loading||{metric_wl}"] = df_wl
    
    # Shop Receiving metrics  
    for metric_sr in ["Not Offloaded", "Total Offloaded", "Old Doc Date", "Same-Day", "WH→Shop Mismatch", "Unique %"]:
        df_sr = load_shop_receiving_drilldown(start_s, end_s, metric_sr)
        if df_sr is not None and not df_sr.empty:
            drilldown_cache[f"🏪 Shop Receiving||{metric_sr}"] = df_sr
    
    # Shop Selling metrics
    for metric_ss in ["Compliance %", "Dup Serials", "No Serial", "Not in WH", "Serial = IC", "Small (≤6)"]:
        df_ss = load_shop_selling_drilldown(start_s, end_s, metric_ss)
        if df_ss is not None and not df_ss.empty:
            drilldown_cache[f"🛒 Shop Selling||{metric_ss}"] = df_ss
...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;
  ...
  <div style="position:relative;width:36px;height:2px;
    ...
    wl_rows = "".join([
        _row("Unique %",        f"{wl_upct:.1f}%",  wl_upct==100, TEAL_P[2],  wl_upct, stage="📦 WH Loading", metric="Unique %"),
        _row("Duplicates",      fmt(wl_dup),         wl_dup==0,  TEAL_P[2],
             100-pct(wl_dup, wl_total) if wl_dup else None, stage="📦 WH Loading", metric="Duplicates"),
        _row("Blank Serials",   fmt(wl_blank),       wl_blank==0, TEAL_P[2],
             100-pct(wl_blank, wl_total) if wl_blank else None, stage="📦 WH Loading", metric="Blank Serials"),
        _row("Small (≤6)",      fmt(wl_small),       wl_small==0, TEAL_P[2],
             100-pct(wl_small, wl_total) if wl_small else None, stage="📦 WH Loading", metric="Small (≤6)"),
    ...
    sr_rows = "".join([
        _row("Not Offloaded",   fmt(sr_notoff),      sr_notoff==0, GREEN_P[2],
             100-pct(sr_notoff, wl_total) if sr_notoff else None, stage="🏪 Shop Receiving", metric="Not Offloaded"),
        _row("Total Offloaded", fmt(sr_total),        True, GREEN_P[2],
               ...
               stage="🏪 Shop Receiving", metric="Total Offloaded"),
        _row("Same-Day",        fmt(sr_same),         True, GREEN_P[2],
             pct(sr_same, sr_total) if sr_total else 100, stage="🏪 Shop Receiving", metric="Same-Day"),
        _row("Old Doc Date",    fmt(sr_old),          sr_old==0, GREEN_P[2],
             100-pct(sr_old, sr_total) if sr_old else None, stage="🏪 Shop Receiving", metric="Old Doc Date"),
           _row("WH→Shop Mismatch",fmt(sr_mmatch),       sr_mmatch==0, GREEN_P[2],
               100-pct(sr_mmatch, sr_total) if sr_mmatch else None, stage="🏪 Shop Receiving", metric="WH→Shop Mismatch"),
        _row("Unique %",        f"{sr_upct:.1f}%",   sr_upct==100, GREEN_P[2],  sr_upct, stage="🏪 Shop Receiving", metric="Unique %"),
    ...
    ss_rows = "".join([
        _row("Compliance %",    f"{ss_pct_v:.1f}%",  ss_pct_v==100, VIOLET_P[2],  ss_pct_v,
             note=f"{fmt(ss_yes)}/{fmt(ss_total)}", stage="🛒 Shop Selling", metric="Compliance %"),
        _row("Dup Serials",     fmt(ss_dup),          ss_dup==0, VIOLET_P[2],
             100-pct(ss_dup, ss_total) if ss_dup else None, stage="🛒 Shop Selling", metric="Dup Serials"),
        _row("No Serial",       fmt(ss_noser),        ss_noser==0, VIOLET_P[2],
             100-pct(ss_noser, ss_total) if ss_noser else None, stage="🛒 Shop Selling", metric="No Serial"),
        _row("Not in WH",       fmt(ss_notwh),        ss_notwh==0, VIOLET_P[2],
             100-pct(ss_notwh, ss_total) if ss_notwh else None, stage="🛒 Shop Selling", metric="Not in WH"),
        _row("IC",              fmt(ss_ic),           ss_ic==0, VIOLET_P[2],
             100-pct(ss_ic, ss_total) if ss_ic else None, stage="🛒 Shop Selling", metric="Serial = IC"),
        _row("Small (≤6)",      fmt(ss_small),        ss_small==0, VIOLET_P[2],
             100-pct(ss_small, ss_total) if ss_small else None, stage="🛒 Shop Selling", metric="Small (≤6)"),
    ])
