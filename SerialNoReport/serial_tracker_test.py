...
"""
MELCOM SERIAL NUMBER TRACKER DASHBOARD
========================================
Objective: Track discrepancies at Warehouse and Shop levels for serialized items

Key Features:
- Warehouse receiving & loading metrics
- Shop offloading patterns & delays
- Discrepancy detection (mismatches, duplicates)
- Sales tracking & fraud detection
- Time-based analysis with breakdown buckets
"""
...
<style>
    ...
    .metric-card.warning {{
        border-left-color: {MELCOM_ORANGE};
    }}
...
def get_warehouse_metrics():
    """Get warehouse receiving and loading metrics"""
...
def render_yesterday_trend(yesterday_by_wh, container=None, include_header=True):
...
@st.cache_data(ttl=300)
def get_loader_breakdown():
    """Get breakdown by warehouse loader"""
    conn = get_db_connection()
    if not conn:
        return None
...
@st.cache_data(ttl=300)
def get_shop_offload_metrics():
    """Get shop offloading metrics with time buckets"""
    conn = get_db_connection()
    if not conn:
        return None
    ...
    offload_data AS (
        SELECT 
            ...
            -- No offloaded (blank MOD date)
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL
                  AND mod_dt IS NULL
            ) as no_offloaded,
    
            -- Offloaded before WH loaded (negative days)
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL 
            ...
            -- Time buckets (shop offload date - WH loaded date)
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL 
                  AND mod_dt IS NOT NULL 
                  AND load_dt IS NOT NULL
...
@st.cache_data(ttl=300)
def get_offloading_details_by_shop_and_time():
...
def get_offloading_pending_by_shop(bucket_label=None, limit=20):
    ...
    ORDER BY pending_offload DESC NULLS LAST, total_loaded DESC
    """
...
def get_sales_metrics():
    """Get sales metrics including duplicates"""
    conn = get_db_connection()
    if not conn:
        return None
...
def main():
    ...
    with tab1:
        ...
        shop_metrics = get_shop_offload_metrics()
        ...
        if wh_metrics and shop_metrics and sales_metrics:
            ...
            default_summary_date = (datetime.today() - timedelta(days=1)).date()
            min_summary_date = datetime(2024, 1, 1).date()
            summary_date = st.date_input(
                "Select GRN Date",
                value=default_summary_date,
                min_value=min_summary_date,
                max_value=default_summary_date,
                key="wh_summary_date_picker",
                help="Choose any GRN date to review warehouse receiving performance."
            )

            if isinstance(summary_date, tuple):
                summary_date = summary_date[0]
            ...
            if summary_overall and summary_overall['total_received'] > 0:
                ...
                if st.session_state.get('show_receiving_duplicates', False) and st.session_state.get('receiving_duplicates_for') == summary_date:
                    ...
                    if dup_details is not None and not dup_details.empty:
                        table_col, trend_col = st.columns([2.5, 1.8], gap="medium")
                        with table_col:
                            st.markdown(
                            ...
                            if st.button("✖ Close Duplicate Details", key=f"close_dup_{summary_date_str}"):
                                ...
                                st.rerun()
                        with trend_col:
                            render_yesterday_trend(summary_by_wh, container=trend_col)
                        trend_rendered_inline = True
                    else:
                        st.info("No duplicate details available for the selected date range.")

                if not trend_rendered_inline:
                    render_yesterday_trend(summary_by_wh)
            else:
            ...
            with col2:
                if sales_metrics['duplicate_sales_count'] > 0:
                    st.markdown(f"""
                    <div class="alert-box critical">
                        <strong>🚨 Potential Fraud: Duplicate Sales</strong><br>
                        {format_number(sales_metrics['unique_duplicate_serials'])} serial numbers sold multiple times ({format_number(sales_metrics['duplicate_sales_count'])} total sales)
                    </div>
                    """, unsafe_allow_html=True)
                
                if not_offloaded and not_offloaded['total_not_offloaded'] > 100:
                    st.markdown(f"""
                    <div class="alert-box warning">
                        <strong>⚠ High Not-Offloaded Count</strong><br>
                        {format_number(not_offloaded['total_not_offloaded'])} items loaded but not offloaded yet
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        wh_metrics = get_warehouse_metrics()
        loader_breakdown = get_loader_breakdown()
    ...
    with tab3:
        ...
        if shop_metrics:
            ...
            with col4:
                if shop_metrics['offload_before_wh_load'] > 0:
                    st.markdown(f"""
                    <div class="metric-card danger" title="Count where offload date < loaded date">
                        <div class="metric-label">⚠ Before WH Load</div>
                        <div class="metric-value">{format_number(shop_metrics['offload_before_wh_load'])}</div>
                        <div class="metric-change negative">Time Anomaly</div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown(f"""
                    <div class="metric-card success" title="Count where offload date < loaded date">
                        <div class="metric-label">✓ Before WH Load</div>
                        <div class="metric-value">0</div>
                        <div class="metric-change positive">No Anomalies</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            min_serial_check_date = date(2024, 1, 1)

            st.markdown("#### Shop-wise Serial Check %")
            st.caption("Select a date range (defaults to yesterday only)")
            serial_check_range = st.date_input(
                "Serial Check Date Range",
                value=(yesterday_date, yesterday_date),
                min_value=min_serial_check_date,
                max_value=yesterday_date,
                key="serial_check_date_range",
                format="YYYY/MM/DD",
                label_visibility="collapsed"
            )

            if isinstance(serial_check_range, tuple) and len(serial_check_range) == 2:
                serial_check_start, serial_check_end = serial_check_range
            else:
                serial_check_start = serial_check_end = serial_check_range
            serial_check_start = max(serial_check_start, min_serial_check_date)
            serial_check_end = min(serial_check_end, yesterday_date)
            if serial_check_start > serial_check_end:
                serial_check_start, serial_check_end = serial_check_end, serial_check_start
            ...
            if not serial_check_metrics:
                ...
                with summary_col:
                    ...
                    if overall:
                        ...
                        st.metric("Total Y", format_number(overall.get('total_yes', 0)))
                ...
                with table_col:
                    if serial_check_metrics.get("missing_shop"):
                        ...
                        if by_shop is not None and not by_shop.empty:
                            ...
                            display_df = by_shop.rename(columns={'yes_pct': 'Compliance %'})
                            pct_min = display_df['Compliance %'].min()
                            pct_max = display_df['Compliance %'].max()

                            def _compliance_gradient(col: pd.Series):
                                if col.name != 'Compliance %':
                                    return [''] * len(col)
                                styles = []
                                if pd.isna(pct_min) or pd.isna(pct_max):
                                    return [''] * len(col)
                                ...
                                for val in col:
                                    if pd.isna(val):
                                        styles.append('')
                                        continue
                                    ratio = (val - pct_min) / span
                                    ratio = max(0, min(1, ratio))
                                    start_rgb = (237, 27, 36)  # MELCOM_RED
                                    end_rgb = (40, 167, 69)    # MELCOM_GREEN
                                    r = int(start_rgb[0] + (end_rgb[0] - start_rgb[0]) * ratio)
                                    g = int(start_rgb[1] + (end_rgb[1] - start_rgb[1]) * ratio)
                                    b = int(start_rgb[2] + (end_rgb[2] - start_rgb[2]) * ratio)
                                    text_color = '#000000' if ratio < 0.5 else '#ffffff'
                                    styles.append(
                                        f'background-color: rgb({r},{g},{b}); color: {text_color}; font-weight:600;'
                                    )
                                return styles
                            ...
                            styled = (
                                ...
                                .apply(_compliance_gradient, axis=0)
                            )
                ...
                with chart_col:
                    ...
                    if serial_check_metrics.get("missing_date"):
                        ...
                        if last11 is not None and not last11.empty:
                            ...
                            fig.update_layout(height=280, yaxis_ticksuffix='%', yaxis_range=[0, 100], margin=dict(l=20, r=10, t=10, b=30))
                            st.plotly_chart(fig, use_container_width=True)
                        else:
                    ...
                    if by_cashier is not None and not by_cashier.empty:
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "total_yes": st.column_config.NumberColumn("Total Y", format="%d"),
            ...
            offload_details = get_offloading_details_by_shop_and_time()
    ...
    with tab_journey:
        ...
        end_date = yesterday
        st.caption("Date range is fixed: 2017/01/01 to yesterday.")
