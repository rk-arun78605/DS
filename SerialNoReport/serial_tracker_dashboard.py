...
def render_compliance_trend_overview(key_suffix=""):
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            fig.update_traces(
                line=dict(color='#1ea89a', width=3),
                marker=dict(size=8, color='#1ea89a', 
            ...
            fig.update_layout(
                ...
                font=dict(family='Source Sans 3', size=12, color='#0b1020'),
                title_font=dict(size=16, family='Space Grotesk', color='#0b1020'),
                hovermode='x unified',
                hoverlabel=dict(
                    bgcolor='white',
                    font_size=12,
...
<style>
    ...
    :root {{
        ...
        --card: #ffffff;
        --border: #e5e7eb;
        --glow: rgba(30, 168, 154, 0.18);
    }}

    .stApp {{
        background: radial-gradient(1200px 600px at 5% 0%, rgba(30, 168, 154, 0.08), transparent 60%),
                    radial-gradient(1200px 600px at 95% 5%, rgba(244, 183, 64, 0.08), transparent 60%),
                    linear-gradient(135deg, #eef2f7 0%, #f8fafc 55%, #edf2f7 100%);
    ...
    /* Dashboard Header */
    .dashboard-header {{
        background: linear-gradient(120deg, #0b3b6d 0%, #0f172a 60%, #0b3b6d 100%);
        padding: 2.2rem 2.5rem;
        border-radius: 18px;
        color: #ffffff;
        margin-bottom: 2rem;
        box-shadow: 0 18px 45px rgba(11, 16, 32, 0.25);
        position: relative;
        overflow: hidden;
    }}
    ...
    .dashboard-header::after {{
        ...
        width: 220px;
        height: 220px;
        background: radial-gradient(circle, rgba(30, 168, 154, 0.35) 0%, transparent 70%);
        opacity: 0.8;
    }}
    ...
    .metric-card.success {{
        border-left: 6px solid {MELCOM_GREEN};
    }}
    ...
    .metric-card:hover {{
        ...
        box-shadow: 0 18px 32px rgba(11, 16, 32, 0.12);
    }}
    ...
    .section-header {{
        ...
        font-size: 1.35rem;
        font-weight: 700;
        box-shadow: 0 10px 20px rgba(11, 16, 32, 0.15);
    ...
    div[data-testid="stDateInput"] > div > div {{
        ...
        border: 2px solid rgba(11, 59, 109, 0.4) !important;
    ...
    .dataframe {{
        border-radius: 12px;
        overflow: hidden;
        box-shadow: 0 10px 24px rgba(11, 16, 32, 0.1);
    }}
    ...
    .alert-box {{
        ...
        box-shadow: 0 10px 20px rgba(11, 16, 32, 0.08);
    }}
    ...
    [data-testid="stMetricValue"] {{
        ...
        color: #0b1020 !important;  /* Very dark for maximum visibility */
    ...
    /* Premium gradient background and borders */
    div[data-testid="stPlotlyChart"] {{
        background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%) !important;
        border: 2px solid transparent !important;
        border-image: linear-gradient(135deg, rgba(30, 168, 154, 0.3), rgba(244, 183, 64, 0.3)) 1 !important;
        border-radius: 16px !important;
        padding: 1.2rem !important;
        box-shadow: 0 12px 32px rgba(11, 16, 32, 0.12),
                    0 4px 12px rgba(30, 168, 154, 0.08) !important;
        transition: all 0.3s ease !important;
    }}

    div[data-testid="stPlotlyChart"]:hover {{
        box-shadow: 0 16px 40px rgba(11, 16, 32, 0.15),
                    0 6px 16px rgba(30, 168, 154, 0.12) !important;
        transform: translateY(-2px) !important;
    }}

    /* Make chart backgrounds slightly tinted */
...
@st.cache_data(ttl=300)
def _get_shop_vehicle_col_expr():
    """Return SQL expression for shop-side vehicle column in serial_no_dailydata."""
    conn = get_db_connection()
    if not conn:
        return "NULL::text"
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE table_schema = 'public'
              AND table_name = 'serial_no_dailydata'
        """)
        cols = {row[0].lower() for row in cur.fetchall()}
        cur.close()
        conn.close()

        if 'vc_vehicle_no_1' in cols:
            return 'vc_vehicle_no_1'
        if 'vc_vehicle_no.1' in cols:
...
def get_not_offloaded_metrics():
    ...
    shop_vehicle_expr = _get_shop_vehicle_col_expr()
...
def get_dept_compliance_data(start_date, end_date):
    ...
    try:
        df_summary = pd.read_sql(query_summary, conn, params=(start_str, end_str))
        df_trend = pd.read_sql(query_trend, conn, params=(str(trend_start), str(trend_end)))
        conn.close()
...
def main():
    ...
    <div class="dashboard-header" data-aos="fade-down">
        <div class="header-grid">
            ...
            <div class="header-actions">
                <div class="action-pill">Owner View: Executive Summary</div>
                <div class="action-pill">Focus: WH to Shop compliance</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown('<div class="section-header" data-aos="fade-up">📅 Warehouse Date Filter</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-shell" data-aos="fade-up">', unsafe_allow_html=True)
    ...
    # Date input styling is handled in global CSS
    ...
    with tab1:
        ...
        with refresh_cols[0]:
            if st.button(
                ...
                help="Clear all cached metrics and reload fresh data from database (including blank serial counts)"
            ):
        ...
        if wh_metrics and shop_metrics:
            ...
            shop_offloaded_unique_pct = (shop_metrics['unique_offloaded'] / shop_metrics['total_offloaded'] * 100) if shop_metrics['total_offloaded'] > 0 else 0
            shop_offloading_pct = (shop_metrics['total_offloaded'] / wh_metrics['loaded_total'] * 100) if wh_metrics['loaded_total'] > 0 else 0
            
            # Determine card classes based on uniqueness %
            # If not 100% unique (and not both 0), use danger class (red)
            wh_received_class = "danger" if (wh_received_unique_pct < 100 and wh_metrics['received_total'] > 0) else "success"
            shop_offloaded_class = "danger" if (shop_offloaded_unique_pct < 100 and shop_metrics['total_offloaded'] > 0) else "success"
            ...
            with col1:
                ...
                st.markdown(f"""
                <div class="metric-card {wh_received_class}" title="Total records in whreceived_serialno (includes blank/empty serial_no)">
            ...
            with col2:
                ...
                st.markdown(f"""
                <div class="metric-card {wh_loaded_class}" title="Total records in serial_no_dailydata (includes blank/empty serial_no)">
            ...
            with col3:
                st.markdown(f"""
                <div class="metric-card {shop_offloaded_class}" title="Total rows with vc_serail_no present in serial_no_dailydata">
                    <div class="metric-label">Shop Offloaded</div>
                    <div class="metric-value">{format_number(shop_metrics['total_offloaded'])}</div>
                    <div class="metric-change">✓ {format_number(shop_metrics['unique_offloaded'])} Unique ({shop_offloaded_unique_pct:.1f}%)</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col4:
                ...
                st.markdown(f"""
                <div class="metric-card {shop_offloading_class}" title="Total offloaded items vs total loaded items">
                    <div class="metric-label">SHOP offloading %</div>
                    <div class="metric-value">{shop_offloading_pct:.1f}%</div>
                    <div class="metric-change" style="{shop_offloading_style}">✓ {format_number(shop_metrics['total_offloaded'])} of {format_number(wh_metrics['loaded_total'])}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with trend_col1:
                st.markdown("<h4 style='font-size: 0.9rem; margin-bottom: 0.5rem;'>WH Received Uniqueness % (Last 7 Days)</h4>", unsafe_allow_html=True)
                ...
                if wh_received_trend is not None and not wh_received_trend.empty:
                    ...
                    fig.update_traces(
                        line=dict(color='#0b3b6d', width=3),
                        marker=dict(size=8, color='#0b3b6d', 
                                   line=dict(color='#ffffff', width=2))
                    )
                    fig.update_layout(
                        height=250, 
            ...
            with trend_col2:
                st.markdown("<h4 style='font-size: 0.9rem; margin-bottom: 0.5rem;'>WH Loaded Uniqueness % (Last 7 Days)</h4>", unsafe_allow_html=True)
                ...
                if wh_loaded_trend is not None and not wh_loaded_trend.empty:
                    ...
                    fig.update_traces(
                        ...
                        marker=dict(size=8, color='#f4b740', 
                                   line=dict(color='#ffffff', width=2))
                    )
                    ...
                    st.info("No trend data available.")
            with trend_col3:
                            st.markdown("<h4 style='font-size: 0.9rem; margin-bottom: 0.5rem;'>Compliance % (Last 7 Days)</h4>", unsafe_allow_html=True)
                            ...
                            if conn:
                                ...
                                try:
                                    ...
                                    if df is not None and not df.empty:
                                        ...
                                        # Update traces for line graph with premium styling
                                        fig.update_traces(
                                            line=dict(color="#1ea89a", width=4),
                                            marker=dict(size=12, color='#1ea89a',
                                                       line=dict(color='#ffffff', width=2)),
                                            text=df['compliance_pct'].apply(lambda x: f'{x:.1f}%'),
                                            textposition="top center",
                                            textfont=dict(size=13, family='Space Grotesk', color='#0b1020')
                                        )
                                        ...
                                        fig.update_layout(
                                            ...
                                            xaxis=dict(
                                                title_font=dict(size=16, family='Source Sans 3'),
                                                tickfont=dict(size=14, family='Source Sans 3')
                                            ),
                                            yaxis=dict(
                                                title_font=dict(size=16, family='Source Sans 3'),
                                                tickfont=dict(size=14, family='Source Sans 3'),
            ...
            # Executive decision signals for owners
            total_loaded = wh_metrics['loaded_total']
            unresolved_offload = max(0, total_loaded - shop_metrics['total_offloaded'])
            unresolved_rate = (unresolved_offload / total_loaded * 100) if total_loaded > 0 else 0
            duplicate_rate = wh_metrics.get('duplicate_percentage', 0)

            issues = {
                'Not Offloaded': discrepancy_summary.get('no_offloading', 0) if discrepancy_summary else 0,
                'Shop Mismatch': discrepancy_summary.get('shop_mismatch', 0) if discrepancy_summary else 0,
                'Serial Mismatch': discrepancy_summary.get('serial_mismatch', 0) if discrepancy_summary else 0,
                'Vehicle Mismatch': discrepancy_summary.get('vehicle_mismatch', 0) if discrepancy_summary else 0,
                'Sold w/o Offload': discrepancy_summary.get('sold_without_offloading', 0) if discrepancy_summary else 0
            }
            top_issue_label, top_issue_count = max(issues.items(), key=lambda x: x[1]) if issues else ("None", 0)
            total_issues = sum(issues.values())
            issue_rate = (total_issues / wh_metrics['received_unique'] * 100) if wh_metrics['received_unique'] > 0 else 0

            st.markdown('<div class="section-header" data-aos="fade-up">🧭 Executive Decision Signals</div>', unsafe_allow_html=True)
            st.markdown(f"""
            <div class="insight-grid">
                <div class="insight-card">
                    <div class="insight-title">Offload Gap</div>
                    <div class="insight-value">{format_number(unresolved_offload)}</div>
                    <div class="insight-note">{unresolved_rate:.1f}% of WH loaded still pending shop offload</div>
                </div>
                <div class="insight-card">
                    <div class="insight-title">Duplicate Risk</div>
                    <div class="insight-value">{duplicate_rate:.1f}%</div>
                    <div class="insight-note">Share of WH loaded serials flagged as duplicates</div>
                </div>
                <div class="insight-card">
                    <div class="insight-title">Compliance Health</div>
                    <div class="insight-value">{format_percentage(comp_val)}</div>
                    <div class="insight-note">Sold serial compliance for selected window</div>
                </div>
                <div class="insight-card">
                    <div class="insight-title">Top Issue</div>
                    <div class="insight-value">{top_issue_label}</div>
                    <div class="insight-note">{format_number(top_issue_count)} occurrences across discrepancy checks</div>
                </div>
            </div>
            """, unsafe_allow_html=True)

            priority_actions = []
            if unresolved_rate > 5:
                priority_actions.append("Close WH-to-shop offload gap; reconcile pending deliveries by shop.")
            if duplicate_rate > 2:
                priority_actions.append("Investigate duplicate serial generation at WH receiving/loading.")
            if comp_val < 90:
                priority_actions.append("Improve POS serial compliance training and exception handling.")
            if issue_rate > 3:
                priority_actions.append("Prioritize root-cause resolution for high discrepancy rate.")
            if not priority_actions:
                priority_actions.append("No critical actions detected; maintain current controls and monitoring.")
            ...
            if summary_overall and summary_overall['total_received'] > 0:
                ...
                if st.session_state.get('show_receiving_duplicates', False) and st.session_state.get('receiving_duplicates_for') == (summary_start, summary_end):
                    ...
                    range_end_default = summary_end
                    min_available_date = datetime(2024, 1, 1).date()
                    range_start_default = max(summary_end.replace(day=1), min_available_date)
                    ...
                    if dup_details is not None and not dup_details.empty:
                        ...
                        with compliance_col:
                            render_compliance_trend_overview(key_suffix="_1")
                        if not trend_rendered_inline:
                            ...
                            with col_compliance:
                                render_compliance_trend_overview(key_suffix="_2")

        # ==================== OVERVIEW COMPLIANCE TREND FUNCTION ====================
                    with col_trend:
                        render_yesterday_trend(summary_by_wh, container=col_trend, key_suffix="_3", include_header=False)
                    with col_compliance:
                        render_compliance_trend_overview(key_suffix="_3")
            else:
        ...
        if not overview_7day_metrics:
            ...
            if True:
                ...
                if by_shop_7day is not None and not by_shop_7day.empty and not selected_shop_overview_compliance:
                    ...
                    if all_shops_7day_result and all_shops_7day_result.get("data") is not None:
                        ...
                        # Gradient styling for combined shop table
                        def _shops_row_gradient(row):
                            ...
                            for idx, col in enumerate(row.index):
                                # Shop Code column
                                if col == 'Shop Code':
                                    styles[idx] = 'font-weight:700; background-color:#f0f9ff;'
                                # Summary columns
                                elif col in ['Total Serials (7d)', 'Total Yes (7d)']:
                                    styles[idx] = 'font-weight:600;'
                                # Compliance % (7d) with special highlight
                                elif col == 'Compliance % (7d)':
                                    val = row.loc[col]
                                    if pd.notna(val):
                                        # Use the same gradient logic for overall compliance
                                        ...
                                        styles[idx] = f'background-color: rgb({r},{g},{b}); color: #000000; font-weight:700;'
                                    else:
                                        styles[idx] = 'font-weight:600;'
                                # Date columns with gradient
                                elif col in date_labels:
                                    val = row.loc[col]
                                    
                                    if pd.isna(val):
                                        styles[idx] = 'background-color: #ffffff; color: #999999;'
                                    else:
                                        if span > 0:
                                            ratio = (float(val) - min_val) / span
                                        else:
                                        ...
                                        styles[idx] = f'background-color: rgb({r},{g},{b}); color: #000000; font-weight:600;'
                                else:
                                    styles[idx] = ''
            ...
            if selected_shop_overview_compliance:
                ...
                # Get single-day and 7-day metrics for selected shop
                by_shop_7day = overview_7day_metrics.get("by_shop")
                if by_shop_7day is not None and not by_shop_7day.empty:
                    shop_df_7day = by_shop_7day[by_shop_7day['shop_code'].str.strip() == selected_shop_overview_compliance.strip()]
                else:
                    shop_df_7day = pd.DataFrame()
                
                # Get end-date metrics for selected shop
                enddate_shop_metrics = enddate_metrics.get("by_shop") if enddate_metrics else None
                shop_df_enddate = None
                if enddate_shop_metrics is not None and not enddate_shop_metrics.empty:
                    shop_df_enddate = enddate_shop_metrics[enddate_shop_metrics['shop_code'].str.strip() == selected_shop_overview_compliance.strip()]
                
                # Display summary metrics in columns (metric cards)
                summary_cols = st.columns([1, 1, 1, 1, 1, 1])
                
                # End-date metrics
                if shop_df_enddate is not None and not shop_df_enddate.empty:
                    total_serials_end = shop_df_enddate.iloc[0]['total_serials']
                    total_yes_end = shop_df_enddate.iloc[0]['total_yes']
                    compliance_pct_end = shop_df_enddate.iloc[0]['yes_pct']
                ...
                # 7-day metrics
                if not shop_df_7day.empty:
                    total_serials_7d = shop_df_7day.iloc[0]['total_serials']
                    total_yes_7d = shop_df_7day.iloc[0]['total_yes']
                    compliance_pct_7d = shop_df_7day.iloc[0]['yes_pct']
                            ...
                            def _row_gradient(row):
                                ...
                                for idx, col in enumerate(row.index):
                                    if col not in date_labels:
                                        if is_shop_row:
                                            styles[idx] = 'font-weight:600; background-color:#e0f2fe; color: #000000;'
                                        else:
                                            styles[idx] = 'font-weight:500;'
                                        continue
                                    
                                    val = row.loc[col]
                                    
                                    if pd.isna(val):
                                        styles[idx] = 'background-color: #ffffff; color: #999999; font-style: italic;'
                                    else:
                                        if span > 0:
                                            ratio = (float(val) - min_val) / span
                                        else:
    ...
    with tab3:
        ...
        if shop_metrics:
            ...
            # ==================== DEPT COMPLIANCE ANALYSIS ====================
            st.markdown('<div class="section-header">📦 Department / Group / Sub-Group Compliance Analysis</div>', unsafe_allow_html=True)
            st.caption(f"📅 Summary: {serial_check_start:%Y/%m/%d} → {serial_check_end:%Y/%m/%d} | Trend: Last 7 Days | Source: mv_dept_compliance")

            df_dept_summary, df_dept_trend = get_dept_compliance_data(serial_check_start, serial_check_end)

            if df_dept_summary is not None and not df_dept_summary.empty:
                # ── Level selector ───────────────────────────────────────────
                level_choice = st.radio(
                    "View by:",
                    ["Department", "Group", "Sub-Group"],
                    horizontal=True,
                    key="dept_compliance_level"
                )
                ...
                # Aggregate summary to selected level
                agg_summary = (
                    df_dept_summary
                ...
                with dept_col1:
                    ...
                    fig_dept_bar.update_layout(
                        ...
                        plot_bgcolor='rgba(248, 250, 252, 0.5)',
                        paper_bgcolor='rgba(0,0,0,0)',
                        font=dict(family='Source Sans 3', size=11, color='#0b1020'),
                        title_font=dict(size=14, family='Space Grotesk', color='#0b1020'),
    ...
    with tab4:
        ...
        st.caption("This tab is currently disabled and will be available soon.")
