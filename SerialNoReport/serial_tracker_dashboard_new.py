...
def get_wh_uniqueness_trend(mode='received'):
    ...
    if mode == 'received':
        ...
        FROM serial_no_dailydata
                WHERE DATE(dt_doc_date) >= '{start_date}'
                    AND DATE(dt_doc_date) <= '{end_date}'
...
def get_warehouse_metrics(received_range=None, loaded_range=None, shop_range=None):
    """Get warehouse metrics filtered by optional date ranges.
    shop_range filters shop_offloaded CTE by dt_mod_date (shop offload date).
    """
    conn = get_db_connection()
    if not conn:
        return None
    ...
    loaded_date_expr = "dt_doc_date::DATE"

    if loaded_range:
        loaded_filter = f" AND {loaded_date_expr} BETWEEN %s AND %s"
        params.extend(loaded_range)

    # shop_offloaded uses dt_mod_date (when shop physically offloaded)
    if shop_range:
        shop_filter = " AND dt_mod_date::DATE BETWEEN %s AND %s"
        params.extend(shop_range)
    ...
    shop_offloaded AS (
        -- Filtered by dt_mod_date (when shop physically offloaded the item)
        SELECT
            COUNT(*) FILTER (
            ...
            -- Serial match: shop offloaded the SAME serial that WH loaded for that row
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL AND LENGTH(TRIM(vc_serail_no)) > 0
                  AND serial_no IS NOT NULL AND LENGTH(TRIM(serial_no)) > 0
                  AND TRIM(vc_serail_no) = TRIM(serial_no)
            ) as wh_serial_matched,
            -- Serial mismatch: shop offloaded a DIFFERENT serial than WH loaded
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL AND LENGTH(TRIM(vc_serail_no)) > 0
                  AND (serial_no IS NULL OR LENGTH(TRIM(serial_no)) = 0
                       OR TRIM(vc_serail_no) != TRIM(serial_no))
            ) as wh_serial_mismatched,
            -- Offloaded same day as WH doc date
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL AND LENGTH(TRIM(vc_serail_no)) > 0
                  AND dt_doc_date IS NOT NULL AND dt_mod_date IS NOT NULL
                  AND dt_mod_date::DATE = dt_doc_date::DATE
            ) as shop_same_day,
            -- Offloaded on a later day than WH doc date (old/pending stock)
            COUNT(*) FILTER (
                WHERE vc_serail_no IS NOT NULL AND LENGTH(TRIM(vc_serail_no)) > 0
                  AND dt_doc_date IS NOT NULL AND dt_mod_date IS NOT NULL
                  AND dt_mod_date::DATE > dt_doc_date::DATE
    ...
    SELECT 
        ...
        s.wh_serial_matched,
        s.wh_serial_mismatched,
        s.shop_same_day,
        s.shop_old_doc,
        CASE
            WHEN (s.wh_serial_matched + s.wh_serial_mismatched) > 0
            THEN ROUND((s.wh_serial_matched::NUMERIC / (s.wh_serial_matched + s.wh_serial_mismatched) * 100), 2)
            ELSE 0
        END as wh_serial_matched_pct,
        CASE 
...
def get_shop_offload_metrics(start_date=None, end_date=None):
    ...
    load_date_expr = "dt_doc_date::DATE"
...
def main():
    ...
    with tab1:
        # Load all metrics
        wh_metrics = get_warehouse_metrics(received_range, loaded_range, shop_range=warehouse_range)
        ...
        if wh_metrics and shop_metrics:
            ...
            # ── WH Zone ribbon ──────────────────────────────────────────────────
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,rgba(6,18,60,0.97) 0%,rgba(12,32,100,0.93) 60%,rgba(8,50,110,0.90) 100%);
                        border-radius:12px 12px 0 0;border-top:3px solid #3b82f6;
                        border-left:1px solid rgba(59,130,246,0.25);border-right:1px solid rgba(59,130,246,0.25);
                        padding:13px 24px 11px 24px;margin-top:16px;text-align:center;">
                <div style="display:inline-flex;align-items:center;gap:10px;justify-content:center;">
                    <span style="font-size:1.12rem;font-weight:700;color:#bfdbfe;letter-spacing:0.05em;">🏭 WAREHOUSE OPERATIONS</span>
                    <span style="color:rgba(99,130,200,0.7);font-size:1rem;">|</span>
                    <span style="font-size:0.84rem;color:#7aa4d4;font-weight:500;">📅 {summary_date_str}</span>
                </div>
            ...
            if summary_overall and summary_overall['total_received'] > 0:
                ...
                _recv_blank_ct  = summary_overall.get('blank_serials', 0)
                _recv_small_ct  = summary_overall.get('small_serials', 0)
                _recv_perfect   = is_100_unique and _recv_blank_ct == 0 and _recv_small_ct == 0
                _recv_sc = "#10b981" if _recv_perfect else "#ef4444"
                _recv_st = "🟢 Perfect" if _recv_perfect else "🔴 Issues Found"
                _recv_blank_pct = (_recv_blank_ct / summary_overall['total_received'] * 100) if summary_overall['total_received'] > 0 else 0
                _recv_small_pct = (_recv_small_ct / summary_overall['total_received'] * 100) if summary_overall['total_received'] > 0 else 0

                # WH tile style helper (blue-tinted)
                def _wh_recv_tile(label, val, sub, ok=True, sub_color=None, tooltip=None):
                    sc = sub_color or ("#10b981" if ok else "#ef4444")
                    tip = f' title="{tooltip}"' if tooltip else ''
                    cur = "help" if tooltip else "default"
                    return (f'<div{tip} style="background:rgba(10,32,100,0.65);border:1px solid rgba(59,130,246,0.45);'
                            f'border-radius:10px;padding:14px 10px;text-align:center;min-height:115px;'
                            f'display:flex;flex-direction:column;justify-content:center;cursor:{cur};">'  
                            f'<div style="font-size:0.72rem;color:#93c5fd;font-weight:600;text-transform:uppercase;'
                            f'letter-spacing:0.06em;margin-bottom:6px;">{label}</div>'
                            f'<div style="font-size:1.55rem;font-weight:700;color:#fff;line-height:1.2;">{val}</div>'
                            f'<div style="font-size:0.76rem;color:{sc};margin-top:6px;font-weight:500;">{sub}</div></div>')

                st.markdown(f"""
                <div style="background:rgba(10,28,88,0.5);border-left:4px solid #3b82f6;
                            border-right:1px solid rgba(59,130,246,0.2);
                            padding:8px 16px 8px 16px;text-align:center;">
                    <div style="display:inline-flex;align-items:center;gap:10px;justify-content:center;">
                        <span style="font-weight:700;font-size:0.95rem;color:#93c5fd;"> WH Receiving</span>
                        <span style="color:rgba(59,130,246,0.5);font-size:1rem;">|</span>
                        <span style="background:{_recv_sc}22;color:{_recv_sc};border:1px solid {_recv_sc}45;
                                     border-radius:20px;padding:2px 12px;font-size:0.82rem;font-weight:700;">Status {_recv_st}</span>
                    </div>
                </div>""", unsafe_allow_html=True)

                col_total, col_wh_dup, col_unique, col_small, col_dupe, col_blank, col_status = st.columns(7)
                ...
                # Query whreceived_serialno for duplicate serial numbers in selected date range
                _wh_db_dup_ct = 0
                _wh_db_dup_conn = get_db_connection()
                if _wh_db_dup_conn:
                    try:
                        _wh_db_dup_df = pd.read_sql(f"""
                            SELECT COUNT(*) AS cnt
                            FROM whreceived_serialno
                            WHERE grn_date BETWEEN '{summary_start.strftime('%Y-%m-%d')}' AND '{summary_end.strftime('%Y-%m-%d')}'
                              AND serial_no IS NOT NULL
                              AND TRIM(serial_no) != ''
                              AND serial_no IN (
                                  SELECT serial_no
                        ...
                        """, _wh_db_dup_conn)
                        _wh_db_dup_conn.close()
                        if _wh_db_dup_df is not None and not _wh_db_dup_df.empty:
                            _wh_db_dup_ct = int(_wh_db_dup_df.iloc[0]['cnt'] or 0)
                    except Exception:
                        if _wh_db_dup_conn:
                            try: _wh_db_dup_conn.close()
                            except: pass

                with col_total:
                    st.markdown(_wh_recv_tile(
                        "Total Received", format_number(summary_overall['total_received']),
                        summary_date_str, ok=True, sub_color="#FFD700"
                    ), unsafe_allow_html=True)

                with col_wh_dup:
                    _wh_db_dup_pct = (_wh_db_dup_ct / summary_overall['total_received'] * 100) if summary_overall['total_received'] > 0 else 0
                    st.markdown(_wh_recv_tile(
                        "Duplicate Serial in WH DB", format_number(_wh_db_dup_ct),
                        "✓ Perfect! None found" if _wh_db_dup_ct == 0 else f"🔴 {_wh_db_dup_pct:.1f}% of total",
                        ok=(_wh_db_dup_ct == 0),
                        tooltip=f"{_wh_db_dup_ct} / {summary_overall['total_received']} received" if _wh_db_dup_ct > 0 else None
                    ), unsafe_allow_html=True)

                with col_unique:
                    # Unique serials for the selected day from whreceived_serialno
                    _recv_uniq_day_ct = 0
                    _recv_uniq_day_conn = get_db_connection()
                    if _recv_uniq_day_conn:
                        try:
                            _recv_uniq_day_df = pd.read_sql(f"""
                                SELECT COUNT(DISTINCT serial_no) AS cnt
                                FROM whreceived_serialno
                                WHERE grn_date = '{summary_end.strftime('%Y-%m-%d')}'
                                  AND serial_no IS NOT NULL
                                  AND TRIM(serial_no) != ''
                            """, _recv_uniq_day_conn)
                            _recv_uniq_day_conn.close()
                            if _recv_uniq_day_df is not None and not _recv_uniq_day_df.empty:
                                _recv_uniq_day_ct = int(_recv_uniq_day_df.iloc[0]['cnt'] or 0)
                        except Exception:
                            if _recv_uniq_day_conn:
                                try: _recv_uniq_day_conn.close()
                                except: pass
                    _recv_uniq_day_total = summary_overall['total_received']
                    _recv_uniq_day_pct = (_recv_uniq_day_ct / _recv_uniq_day_total * 100) if _recv_uniq_day_total > 0 else 0.0
                    st.markdown(_wh_recv_tile(
                        "Unique Serials", format_number(_recv_uniq_day_ct),
                        f"✓ {_recv_uniq_day_pct:.1f}%" if _recv_uniq_day_pct == 100.0 else f"🔴 {_recv_uniq_day_pct:.1f}%",
                        ok=(_recv_uniq_day_pct == 100.0),
                        tooltip=f"{_recv_uniq_day_ct} unique / {_recv_uniq_day_total} received on {summary_end.strftime('%d %b %Y')}"
                    ), unsafe_allow_html=True)

                with col_small:
                    st.markdown(_wh_recv_tile(
                        "Small Serials (≤6 chars)", format_number(_recv_small_ct),
                        f"{'✓ None found' if _recv_small_ct==0 else f'🔴 {_recv_small_pct:.1f}% of total'}",
                        ok=(_recv_small_ct == 0),
                        tooltip=f"{_recv_small_ct} / {summary_overall['total_received']} received" if _recv_small_ct > 0 else None
                    ), unsafe_allow_html=True)
                ...
                with col_dupe:
                    if summary_overall['duplicate_count'] > 0:
                        ...
                        st.markdown(_wh_recv_tile(
                            "Duplicates", format_number(summary_overall['duplicate_count']),
                            "⚠ Click below to review", ok=False
                        ), unsafe_allow_html=True)
                        if st.button(
                            f"🔍 View {format_number(summary_overall['duplicate_count'])} Duplicates",
                ...
                with col_blank:
                    ...
                    if has_blanks:
                        ...
                        st.markdown(
                            f'<a href="{blank_href}" target="_self" style="text-decoration:none;">'
                            + _wh_recv_tile("Blank Serials", format_number(blank_count),
                                           f"🔴 {blank_pct:.1f}% of total — click to view", ok=False,
                                           tooltip=f"{blank_count} / {summary_overall['total_received']} received")
                            + '</a>', unsafe_allow_html=True
                        )
                    else:
                        st.markdown(_wh_recv_tile(
                            "Blank Serials", format_number(blank_count),
                            "✓ None found", ok=True
                        ), unsafe_allow_html=True)

                with col_status:
                    _recv_status_label = "🟢 Perfect" if _recv_perfect else "🔴 Action Needed"
                    _recv_uniq_pct = summary_overall['unique_percentage']
                    st.markdown(_wh_recv_tile(
                        "Status",
                        _recv_status_label,
                        f"{_recv_uniq_pct:.1f}% Unique",
                        ok=_recv_perfect,
                        sub_color="#10b981" if _recv_perfect else "#ef4444",
                        tooltip=f"{summary_overall['unique_serials']} unique / {summary_overall['total_received']} received"
                    ), unsafe_allow_html=True)
            ...
            # ==================== WH LOADING SECTION ====================
            if wh_metrics:
                _wl_total  = int(wh_metrics.get('loaded_total', 0))
                _wl_unique = int(wh_metrics.get('loaded_unique', 0))
                _wl_dup    = int(wh_metrics.get('duplicate_serials_count', 0))
                _wl_blank  = int(wh_metrics.get('loaded_blank', 0))
                _wl_small  = int(wh_metrics.get('loaded_small', 0))
                _wl_pct    = float(wh_metrics.get('unique_percentage', 0))
                _wl_blank_pct = (_wl_blank / _wl_total * 100) if _wl_total > 0 else 0
                _wl_small_pct = (_wl_small / _wl_total * 100) if _wl_total > 0 else 0
                _wl_perfect = (_wl_pct == 100.0 and _wl_blank == 0 and _wl_small == 0)
                _wl_sc = "#10b981" if _wl_perfect else "#ef4444"
                _wl_st = "🟢 Perfect" if _wl_perfect else "🔴 Issues Found"

                # WH Loading tile style (teal-blue tinted, distinct from Receiving)
                def _wh_load_tile(label, val, sub, ok=True, sub_color=None, tooltip=None):
                    sc = sub_color or ("#10b981" if ok else "#ef4444")
                    tip = f' title="{tooltip}"' if tooltip else ''
                    cur = "help" if tooltip else "default"
                    return (f'<div{tip} style="background:rgba(6,50,80,0.68);border:1px solid rgba(20,184,166,0.45);'
                            f'border-radius:10px;padding:14px 10px;text-align:center;min-height:115px;'
                            f'display:flex;flex-direction:column;justify-content:center;cursor:{cur};">'  
                            f'<div style="font-size:0.72rem;color:#5eead4;font-weight:600;text-transform:uppercase;'
                            f'letter-spacing:0.06em;margin-bottom:6px;">{label}</div>'
                            f'<div style="font-size:1.55rem;font-weight:700;color:#fff;line-height:1.2;">{val}</div>'
                            f'<div style="font-size:0.76rem;color:{sc};margin-top:6px;font-weight:500;">{sub}</div></div>')

                st.markdown(f"""
                <div style="background:rgba(6,40,60,0.55);border-left:4px solid #14b8a6;
                            border-right:1px solid rgba(20,184,166,0.2);padding:8px 16px 8px 16px;
                            text-align:center;">
                    <div style="display:inline-flex;align-items:center;gap:10px;justify-content:center;">
                        <span style="font-weight:700;font-size:0.95rem;color:#5eead4;"> WH Loading</span>
                        <span style="color:rgba(20,184,166,0.5);font-size:1rem;">|</span>
                        <span style="background:{_wl_sc}22;color:{_wl_sc};border:1px solid {_wl_sc}45;
                                     border-radius:20px;padding:2px 12px;font-size:0.82rem;font-weight:700;">Status {_wl_st}</span>
                    </div>
                </div>""", unsafe_allow_html=True)

                _wl_c1, _wl_c2, _wl_c3, _wl_c4, _wl_c5 = st.columns(5)
                with _wl_c1:
                    st.markdown(_wh_load_tile(
                        "Total Loaded", format_number(_wl_total),
                        summary_date_str, ok=True, sub_color="#FFD700"
                    ), unsafe_allow_html=True)
                with _wl_c2:
                    st.markdown(_wh_load_tile(
                        "Duplicates", format_number(_wl_dup),
                        "✓ None found" if _wl_dup == 0 else f"🔴 {_wl_dup:,} found",
                        ok=(_wl_dup == 0),
                        tooltip=f"{_wl_dup} / {_wl_total} loaded" if _wl_dup > 0 else None
                    ), unsafe_allow_html=True)
                with _wl_c3:
                    st.markdown(_wh_load_tile(
                        "Blank Serials", format_number(_wl_blank),
                        "✓ None found" if _wl_blank == 0 else f"🔴 {_wl_blank_pct:.1f}% of total",
                        ok=(_wl_blank == 0),
                        tooltip=f"{_wl_blank} / {_wl_total} loaded" if _wl_blank > 0 else None
                    ), unsafe_allow_html=True)
                with _wl_c4:
                    st.markdown(_wh_load_tile(
                        "Small Serials (≤6 chars)", format_number(_wl_small),
                        "✓ None found" if _wl_small == 0 else f"🔴 {_wl_small_pct:.1f}% of total",
                        ok=(_wl_small == 0),
                        tooltip=f"{_wl_small} / {_wl_total} loaded" if _wl_small > 0 else None
                    ), unsafe_allow_html=True)
                with _wl_c5:
                    st.markdown(_wh_load_tile(
                        "Status", "🟢 Perfect" if _wl_perfect else "🔴 Issues",
                        f"{_wl_pct:.1f}% Unique",
                        ok=_wl_perfect,
                        tooltip=f"{_wl_unique} unique / {_wl_total} loaded"
                    ), unsafe_allow_html=True)

                # Closing WH zone bottom border
                st.markdown('<div style="border-bottom:2px solid rgba(59,130,246,0.3);border-radius:0 0 12px 12px;height:6px;background:rgba(8,22,72,0.5);margin-bottom:6px;"></div>', unsafe_allow_html=True)

            # ==================== SHOP SECTION ====================
            if wh_metrics:
                # Total Offloaded: rows where dt_doc_date is in selected range AND dt_mod_date IS NOT NULL
                _sr_total = 0
                _sr_total_conn = get_db_connection()
                if _sr_total_conn:
                    try:
                        _sr_total_df = pd.read_sql(f"""
                            SELECT COUNT(*) AS cnt
                            FROM serial_no_dailydata
                            WHERE dt_doc_date::DATE
                                  BETWEEN '{summary_start.strftime('%Y-%m-%d')}' AND '{summary_end.strftime('%Y-%m-%d')}'
                              AND dt_mod_date IS NOT NULL
                              AND TRIM(dt_mod_date::TEXT) != ''
                        """, _sr_total_conn)
                        _sr_total_conn.close()
                        if _sr_total_df is not None and not _sr_total_df.empty:
                            _sr_total = int(_sr_total_df.iloc[0]['cnt'] or 0)
                    except Exception:
                        if _sr_total_conn:
                            try: _sr_total_conn.close()
                            except: pass

                # Unique Serials: DISTINCT vc_serail_no among offloaded rows for the selected doc date
                _sr_unique = 0
                _sr_uniq_conn = get_db_connection()
                if _sr_uniq_conn:
                    try:
                        _sr_uniq_df = pd.read_sql(f"""
                            SELECT COUNT(DISTINCT vc_serail_no) AS cnt
                            FROM serial_no_dailydata
                            WHERE dt_doc_date::DATE
                                  BETWEEN '{summary_start.strftime('%Y-%m-%d')}' AND '{summary_end.strftime('%Y-%m-%d')}'
                              AND dt_mod_date IS NOT NULL
                        ...
                        """, _sr_uniq_conn)
                        _sr_uniq_conn.close()
                        if _sr_uniq_df is not None and not _sr_uniq_df.empty:
                            _sr_unique = int(_sr_uniq_df.iloc[0]['cnt'] or 0)
                    except Exception:
                        if _sr_uniq_conn:
                            try: _sr_uniq_conn.close()
                            except: pass
                _sr_pct = (_sr_unique / _sr_total * 100) if _sr_total > 0 else 0.0

                _sr_dup      = int(wh_metrics.get('shop_offload_duplicate', 0))
                _sr_blank    = int(wh_metrics.get('shop_offload_blank', 0))
                _sr_small    = int(wh_metrics.get('shop_offload_small', 0))
                _sr_matched  = int(wh_metrics.get('wh_serial_matched', 0))
                _sr_mismatch = int(wh_metrics.get('wh_serial_mismatched', 0))
                _sr_match_pct= float(wh_metrics.get('wh_serial_matched_pct', 0))
                _sr_same_day = int(wh_metrics.get('shop_same_day', 0))
                # Old Doc Date: doc_date in selected range, dt_mod_date NOT NULL,
                # but dt_mod_date falls OUTSIDE the selected date range
                _sr_old_doc = 0
                _old_doc_conn = get_db_connection()
                if _old_doc_conn:
                    try:
                        _old_doc_df = pd.read_sql(f"""
                            SELECT COUNT(*) AS cnt
                            FROM serial_no_dailydata
                            WHERE dt_doc_date::DATE
                                  BETWEEN '{summary_start.strftime('%Y-%m-%d')}' AND '{summary_end.strftime('%Y-%m-%d')}'
                              AND dt_mod_date IS NOT NULL
                              AND TRIM(dt_mod_date::TEXT) != ''
                              AND dt_mod_date::DATE NOT BETWEEN '{summary_start.strftime('%Y-%m-%d')}' AND '{summary_end.strftime('%Y-%m-%d')}'
                        """, _old_doc_conn)
                        _old_doc_conn.close()
                        if _old_doc_df is not None and not _old_doc_df.empty:
                            _sr_old_doc = int(_old_doc_df.iloc[0]['cnt'] or 0)
                    except Exception:
                        if _old_doc_conn:
                            try: _old_doc_conn.close()
                            except: pass
                _sr_blank_pct= (_sr_blank / (_sr_total + _sr_blank) * 100) if (_sr_total + _sr_blank) > 0 else 0
                _sr_small_pct= (_sr_small / _sr_total * 100) if _sr_total > 0 else 0
                _sr_same_pct = (_sr_same_day / _sr_total * 100) if _sr_total > 0 else 0
                _sr_old_pct  = (_sr_old_doc / _sr_total * 100) if _sr_total > 0 else 0

                # Pre-calculate _not_offloaded here so it feeds into _sr_perfect
                _not_offloaded = 0
                _not_off_conn = get_db_connection()
                if _not_off_conn:
                    try:
                        _not_off_df = pd.read_sql(f"""
                            SELECT COUNT(*) AS cnt
                            FROM serial_no_dailydata
                            WHERE dt_doc_date::DATE
                                  BETWEEN '{summary_start.strftime('%Y-%m-%d')}' AND '{summary_end.strftime('%Y-%m-%d')}'
                              AND (dt_mod_date IS NULL OR TRIM(dt_mod_date::TEXT) = '')
                        """, _not_off_conn)
                        _not_off_conn.close()
                        if _not_off_df is not None and not _not_off_df.empty:
                            _not_offloaded = int(_not_off_df.iloc[0]['cnt'] or 0)
                    except Exception:
                        if _not_off_conn:
                            try: _not_off_conn.close()
                            except: pass

                # Perfect only if ALL metrics are clean
                _sr_perfect  = (
                    _sr_pct == 100.0
                    and _sr_mismatch == 0
                    and _sr_small == 0
                    and _sr_blank == 0
                    and _sr_old_doc == 0
                    and _not_offloaded == 0
                )
                _sr_sc = "#10b981" if _sr_perfect else "#ef4444"
                _sr_st = "🟢 Perfect" if _sr_perfect else "🔴 Issues Found"

                # Shop tile style — dark emerald, clearly different from WH blue zone
                def _shop_tile(label, val, sub, ok=True, sub_color=None, tooltip=None):
                    sc = sub_color or ("#6ee7b7" if ok else "#fca5a5")
                    tip = f' title="{tooltip}"' if tooltip else ''
                    cur = "help" if tooltip else "default"
                    return (f'<div{tip} style="background:rgba(4,44,32,0.75);border:1px solid rgba(16,185,129,0.45);'
                            f'border-radius:10px;padding:12px 8px;text-align:center;min-height:115px;'
                            f'display:flex;flex-direction:column;justify-content:center;cursor:{cur};">'
                            f'<div style="font-size:0.68rem;color:#6ee7b7;font-weight:600;text-transform:uppercase;'
                            f'letter-spacing:0.06em;margin-bottom:6px;">{label}</div>'
                            f'<div style="font-size:1.45rem;font-weight:700;color:#fff;line-height:1.2;">{val}</div>'
                            f'<div style="font-size:0.73rem;color:{sc};margin-top:5px;font-weight:500;">{sub}</div></div>')

                # Smaller variant for sub-metrics under Total Offloaded (Same Day, Old Doc Date)
                def _shop_tile_sm(label, val, sub, ok=True, sub_color=None, tooltip=None):
                    sc = sub_color or ("#6ee7b7" if ok else "#fca5a5")
                    tip = f' title="{tooltip}"' if tooltip else ''
                    cur = "help" if tooltip else "default"
                    return (f'<div{tip} style="background:rgba(4,36,26,0.70);border:1px solid rgba(16,185,129,0.32);'
                            f'border-radius:8px;padding:9px 6px;text-align:center;min-height:95px;'
                            f'display:flex;flex-direction:column;justify-content:center;cursor:{cur};">'
                            f'<div style="font-size:0.61rem;color:#86efac;font-weight:600;text-transform:uppercase;'
                            f'letter-spacing:0.05em;margin-bottom:4px;">{label}</div>'
                            f'<div style="font-size:1.18rem;font-weight:700;color:#e2e8f0;line-height:1.2;">{val}</div>'
                            f'<div style="font-size:0.67rem;color:{sc};margin-top:4px;font-weight:500;">{sub}</div></div>')

                # Extra-small variant for quality/purity metrics (Mismatch, Unique, Blank, Small)
                def _shop_tile_xs(label, val, sub, ok=True, sub_color=None, tooltip=None):
                    sc = sub_color or ("#6ee7b7" if ok else "#fca5a5")
                    tip = f' title="{tooltip}"' if tooltip else ''
                    cur = "help" if tooltip else "default"
                    return (f'<div{tip} style="background:rgba(3,28,18,0.65);border:1px solid rgba(16,185,129,0.22);'
                            f'border-radius:7px;padding:7px 5px;text-align:center;min-height:78px;'
                            f'display:flex;flex-direction:column;justify-content:center;cursor:{cur};">'
                            f'<div style="font-size:0.56rem;color:#6ee7b7;font-weight:600;text-transform:uppercase;'
                            f'letter-spacing:0.04em;margin-bottom:3px;">{label}</div>'
                            f'<div style="font-size:1.0rem;font-weight:700;color:#cbd5e1;line-height:1.2;">{val}</div>'
                            f'<div style="font-size:0.62rem;color:{sc};margin-top:3px;font-weight:500;">{sub}</div></div>')

                st.markdown(f"""
                <div style="background:linear-gradient(135deg,rgba(2,28,20,0.97) 0%,rgba(4,52,36,0.93) 60%,rgba(3,44,28,0.90) 100%);
                            border-radius:12px 12px 0 0;border-top:3px solid #10b981;
                            border-left:1px solid rgba(16,185,129,0.25);border-right:1px solid rgba(16,185,129,0.25);
                            padding:13px 24px 11px 24px;margin-top:20px;text-align:center;">
                    <div style="display:inline-flex;align-items:center;gap:10px;justify-content:center;">
                        <span style="font-size:1.12rem;font-weight:700;color:#a7f3d0;letter-spacing:0.05em;">🏪 SHOP OPERATIONS</span>
                        <span style="color:rgba(52,160,100,0.7);font-size:1rem;">|</span>
                        <span style="font-size:0.84rem;color:#6ee7b7;font-weight:500;">📅 {summary_date_str}</span>
                        <span style="color:rgba(52,160,100,0.7);font-size:1rem;">|</span>
                        <span style="background:{_sr_sc}22;color:{_sr_sc};border:1px solid {_sr_sc}45;
                                     border-radius:20px;padding:2px 12px;font-size:0.82rem;font-weight:700;">{_sr_st}</span>
                    </div>
                </div>
                <div style="background:rgba(4,36,26,0.6);border-left:4px solid #059669;
                            border-right:1px solid rgba(5,150,105,0.2);
                            padding:8px 16px 8px 16px;text-align:center;">
                    <div style="display:inline-flex;align-items:center;gap:10px;justify-content:center;">
                        <span style="font-weight:700;font-size:0.92rem;color:#6ee7b7;">📥 Shop Receiving</span>
                        <span style="color:rgba(5,150,105,0.5);font-size:1rem;">|</span>
                        <span style="font-size:0.78rem;color:#94a3b8;font-weight:400;">Shop Received Date: {summary_date_str}</span>
                    </div>
                </div>""", unsafe_allow_html=True)

                # Not Offloaded already calculated above before the ribbon — just compute pct
                _not_off_pct = (_not_offloaded / _wl_total * 100) if _wl_total > 0 else 0

                _s0, _sno, _s1, _s2, _s3, _s4, _s5, _s6, _s7 = st.columns(9)
                with _s0:
                    st.markdown(_shop_tile(
                        "Total Sent to Shop", format_number(_wl_total),
                        summary_date_str, ok=True, sub_color="#FFD700"
                    ), unsafe_allow_html=True)
                with _sno:
                    st.markdown(_shop_tile(
                        "Not Offloaded", format_number(_not_offloaded),
                        "✓ All offloaded" if _not_offloaded == 0 else f"🔴 {_not_off_pct:.1f}% pending",
                        ok=(_not_offloaded == 0),
                        tooltip=f"{_not_offloaded} / {_wl_total} sent not yet offloaded" if _not_offloaded > 0 else None
                    ), unsafe_allow_html=True)
                with _s1:
                    st.markdown(_shop_tile(
                        "Total Offloaded", format_number(_sr_total),
                        f"{100 - _not_off_pct:.1f}% of sent", ok=True, sub_color="#6ee7b7",
                        tooltip=f"{_sr_total} / {_wl_total} sent to shop"
                    ), unsafe_allow_html=True)
                with _s2:
                    st.markdown(_shop_tile_sm(
                        "Same Day Offloaded", format_number(_sr_same_day),
                        f"✓ {_sr_same_pct:.1f}% of total",
                        ok=True, sub_color="#6ee7b7",
                        tooltip=f"{_sr_same_day} / {_sr_total} offloaded"
                    ), unsafe_allow_html=True)
                with _s3:
                    _old_doc_pct_of_sent = (_sr_old_doc / _wl_total * 100) if _wl_total > 0 else 0
                    st.markdown(_shop_tile_sm(
                        "Old Doc Date", format_number(_sr_old_doc),
                        f"🔴 {_old_doc_pct_of_sent:.1f}% of sent" if _sr_old_doc > 0 else "✓ None",
                        ok=(_sr_old_doc == 0),
                        tooltip=f"{_sr_old_doc} / {_wl_total} sent to shop" if _sr_old_doc > 0 else None
                    ), unsafe_allow_html=True)
                with _s4:
                    st.markdown(_shop_tile_xs(
                        "WH → Shop Mismatch", format_number(_sr_mismatch),
                        f"✓ All matched ({_sr_match_pct:.1f}%)" if _sr_mismatch == 0
                        else f"🔴 {100 - _sr_match_pct:.1f}% mismatch",
                        ok=(_sr_mismatch == 0),
                        tooltip=f"{_sr_mismatch} mismatched / {_sr_total} offloaded" if _sr_mismatch > 0 else f"{_sr_matched} / {_sr_total} matched"
                    ), unsafe_allow_html=True)
                with _s5:
                    st.markdown(_shop_tile_xs(
                        "Unique Serials", format_number(_sr_unique),
                        f"✓ {_sr_pct:.1f}%" if _sr_pct == 100.0 else f"🔴 {_sr_pct:.1f}%",
                        ok=(_sr_pct == 100.0),
                        tooltip=f"{_sr_unique} unique / {_sr_total} offloaded"
                    ), unsafe_allow_html=True)
                with _s6:
                    st.markdown(_shop_tile_xs(
                        "Blank Serials", format_number(_sr_blank),
                        "✓ None" if _sr_blank == 0 else f"🔴 {_sr_blank_pct:.1f}% of total",
                        ok=(_sr_blank == 0),
                        tooltip=f"{_sr_blank} / {_sr_total} offloaded" if _sr_blank > 0 else None
                    ), unsafe_allow_html=True)
                with _s7:
                    st.markdown(_shop_tile_xs(
                        "Small Serials (≤6 chars)", format_number(_sr_small),
                        "✓ None" if _sr_small == 0 else f"🔴 {_sr_small_pct:.1f}% of total",
                        ok=(_sr_small == 0),
                        tooltip=f"{_sr_small} / {_sr_total} offloaded" if _sr_small > 0 else None
                    ), unsafe_allow_html=True)

                # ---- Shop Selling sub-section ----
                _sell_conn = get_db_connection()
                _sell_total = _sell_yes = _sell_dupe = _sell_no_serial = _sell_small_s = _sell_not_wh = _sell_serial_is_ic = 0
                _sell_pct = 0.0
                if _sell_conn:
                    try:
                        _sell_start_s = summary_start.strftime('%Y-%m-%d')
                        _sell_end_s   = summary_end.strftime('%Y-%m-%d')
                        _sell_df = pd.read_sql(f"""
                            WITH base AS (
                                SELECT item_code, serial_number, serial_check
                                FROM serialno_check_yes_no
                                WHERE DATE(bill_date) BETWEEN '{_sell_start_s}' AND '{_sell_end_s}'
                            ...
                            SELECT
                                ...
                                COUNT(*) FILTER (WHERE serial_number IN (SELECT serial_number FROM dup_serials)) AS duplicate_sold,
                                COUNT(*) FILTER (WHERE serial_number IS NULL OR TRIM(COALESCE(serial_number,'')) = '') AS sold_no_serial,
                                COUNT(*) FILTER (
                                    WHERE serial_number IS NOT NULL
                                      AND LENGTH(TRIM(COALESCE(serial_number,''))) > 0
                                      AND LENGTH(TRIM(serial_number)) <= 6
                                )                                                                            AS small_serial_sold,
                                COUNT(*) FILTER (
                                ...
                                COUNT(*) FILTER (
                                    WHERE serial_number IS NOT NULL
                                      AND TRIM(COALESCE(serial_number,'')) != ''
                                      AND REGEXP_REPLACE(TRIM(serial_number), '[^0-9]', '', 'g') != ''
                                      AND REGEXP_REPLACE(TRIM(serial_number), '[^0-9]', '', 'g') = TRIM(item_code)
                                )                                                                            AS serial_is_itemcode
                            FROM base
                        """, _sell_conn)
                        _sell_conn.close()
                        if _sell_df is not None and not _sell_df.empty:
                            _sell_total       = int(_sell_df.iloc[0]['total_sold']       or 0)
                            _sell_yes         = int(_sell_df.iloc[0]['compliance_yes']   or 0)
                            _sell_dupe        = int(_sell_df.iloc[0]['duplicate_sold']   or 0)
                            _sell_no_serial   = int(_sell_df.iloc[0]['sold_no_serial']   or 0)
                            _sell_small_s     = int(_sell_df.iloc[0]['small_serial_sold']or 0)
                            _sell_not_wh      = int(_sell_df.iloc[0]['not_in_wh']        or 0)
                            _sell_serial_is_ic= int(_sell_df.iloc[0]['serial_is_itemcode']or 0)
                            _sell_pct         = round(_sell_yes / _sell_total * 100, 1) if _sell_total > 0 else 0.0
                    except Exception as _sell_err:
                        if _sell_conn:
                            try: _sell_conn.close()
                            except: pass
                        st.error(f"Shop Selling query error: {_sell_err}")

                def _spct(num, denom):
                    return round(num / denom * 100, 1) if denom > 0 else 0.0

                st.markdown(f"""
                <div style="background:rgba(3,30,20,0.65);border-left:4px solid #10b981;
                            border-right:1px solid rgba(16,185,129,0.2);
                            padding:8px 16px 8px 16px;text-align:center;margin-top:2px;">
                    <div style="display:inline-flex;align-items:center;gap:10px;justify-content:center;">
                        <span style="font-weight:700;font-size:0.92rem;color:#a7f3d0;">🛒 Shop Selling</span>
                        <span style="color:rgba(16,185,129,0.5);font-size:1rem;">|</span>
                        <span style="font-size:0.78rem;color:#94a3b8;font-weight:400;">BILL_DATE: {summary_date_str}</span>
                    </div>
                </div>""", unsafe_allow_html=True)

                _ss0, _ss1, _ss2, _ss3, _ss4, _ss5, _ss6 = st.columns(7)
                with _ss0:
                    st.markdown(_shop_tile(
                        "Total Sold with Serial No", format_number(_sell_total),
                        summary_date_str, ok=True, sub_color="#FFD700"
                    ), unsafe_allow_html=True)
                with _ss1:
                    st.markdown(_shop_tile(
                        "Compliance %", f"{_sell_pct:.1f}%",
                        "✓ All compliant" if _sell_pct == 100.0 else f"🔴 {format_number(_sell_yes)} of {format_number(_sell_total)}",
                        ok=(_sell_pct == 100.0),
                        tooltip=f"{_sell_not_wh} not in WH records / {_sell_total} sold"
                    ), unsafe_allow_html=True)
                with _ss2:
                    _sell_dupe_pct = _spct(_sell_dupe, _sell_total)
                    st.markdown(_shop_tile(
                        "Duplicate Serials", format_number(_sell_dupe),
                        "✓ None" if _sell_dupe == 0 else f"🔴 {_sell_dupe_pct:.1f}% of total",
                        ok=(_sell_dupe == 0),
                        tooltip=f"{_sell_dupe} / {_sell_total} sold" if _sell_dupe > 0 else None
                    ), unsafe_allow_html=True)
                with _ss3:
                    _sell_no_pct = _spct(_sell_no_serial, _sell_total)
                    st.markdown(_shop_tile(
                        "No Serial Number", format_number(_sell_no_serial),
                        "✓ None" if _sell_no_serial == 0 else f"🔴 {_sell_no_pct:.1f}% of total",
                        ok=(_sell_no_serial == 0),
                        tooltip=f"{_sell_no_serial} / {_sell_total} sold" if _sell_no_serial > 0 else None
                    ), unsafe_allow_html=True)
                with _ss4:
                    _sell_small_pct = _spct(_sell_small_s, _sell_total)
                    st.markdown(_shop_tile(
                        "Small Serials (≤6)", format_number(_sell_small_s),
                        "✓ None" if _sell_small_s == 0 else f"🔴 {_sell_small_pct:.1f}% of total",
                        ok=(_sell_small_s == 0),
                        tooltip=f"{_sell_small_s} / {_sell_total} sold" if _sell_small_s > 0 else None
                    ), unsafe_allow_html=True)
                with _ss5:
                    _sell_notwh_pct = _spct(_sell_not_wh, _sell_total)
                    st.markdown(_shop_tile(
                        "Not in WH Records", format_number(_sell_not_wh),
                        "✓ All matched" if _sell_not_wh == 0 else f"🔴 {_sell_notwh_pct:.1f}% of total",
                        ok=(_sell_not_wh == 0),
                        tooltip=f"{_sell_not_wh} / {_sell_total} sold" if _sell_not_wh > 0 else None
                    ), unsafe_allow_html=True)
                with _ss6:
                    _sell_ic_pct = _spct(_sell_serial_is_ic, _sell_total)
                    st.markdown(_shop_tile(
                        "Serial = Item Code", format_number(_sell_serial_is_ic),
                        "✓ None" if _sell_serial_is_ic == 0 else f"🔴 {_sell_ic_pct:.1f}% of total",
                        ok=(_sell_serial_is_ic == 0),
                        tooltip=f"{_sell_serial_is_ic} / {_sell_total} sold" if _sell_serial_is_ic > 0 else None
                    ), unsafe_allow_html=True)

                # ---- Offloading Time Slab Trend (collapsed by default) ----
                _SLAB_COLS     = ["Before WH Loaded", "0 Days", "1-3 Days", "4-7 Days", "8-10 Days", ">10 Days"]
                _SLAB_SQL_CONDS = {
                ...
                _slab_trend_start = summary_end - timedelta(days=10)
                _slab_trend_end   = summary_end
                _slab_tbl_key     = f"slab_tbl_{summary_end.strftime('%Y%m%d')}"
                _slab_click_key   = f"slab_click_{summary_end.strftime('%Y%m%d')}"

                st.markdown("""
                <style>
                /* Dark theme for Offloading Slab expander (and all expanders in this dark app) */
                div[data-testid="stExpander"] > details {
                    background: rgba(3,30,20,0.75) !important;
                ...
                div[data-testid="stExpander"] > details > summary {
                    background: rgba(3,36,24,0.90) !important;
                ...
                div[data-testid="stExpander"] > details > summary:hover {
                    background: rgba(4,52,36,0.95) !important;
                ...
                div[data-testid="stExpander"] > details > div[data-testid="stExpanderDetails"] {
                    background: rgba(2,24,16,0.82) !important;
                ...
                """, unsafe_allow_html=True)
                with st.expander(
                    f"⏱ Offloading Time Slab Trend  |  Shop Received Date (MOD Date): "
                    f"{_slab_trend_start.strftime('%d %b %Y')} → {_slab_trend_end.strftime('%d %b %Y')}  |  "
                    f"Click any cell to drill down by shop",
                    expanded=False
                ):
                    _slab_trend_df = None
                    _slab_conn = get_db_connection()
                    if _slab_conn:
                        try:
                            _slab_trend_df = pd.read_sql(f"""
                                SELECT
                                ...
                                WHERE dt_mod_date IS NOT NULL
                                  ...
                                  AND dt_mod_date::DATE BETWEEN '{_slab_trend_start.strftime('%Y-%m-%d')}' AND '{_slab_trend_end.strftime('%Y-%m-%d')}'
                                GROUP BY dt_mod_date::DATE
                                ORDER BY dt_mod_date::DATE DESC
                            """, _slab_conn)
                            _slab_conn.close()
                        except Exception as _slab_err:
                            if _slab_conn:
                                try: _slab_conn.close()
                                except: pass
                            st.warning(f"⚠ Offloading slab query error: {_slab_err}")

                    if _slab_trend_df is not None and not _slab_trend_df.empty:
                        _slab_raw_dates = list(_slab_trend_df['mod_date'])
                        _slab_disp = _slab_trend_df.copy()
                        _slab_disp['mod_date'] = pd.to_datetime(_slab_disp['mod_date']).dt.strftime('%d %b %Y (%a)')
                        _slab_disp = _slab_disp.rename(columns={'mod_date': 'MOD Date'}).set_index('MOD Date')

                        def _slab_cell_colour(val, col_name, col_max):
                        ...
                        def _style_slab_df(df):
                            s = df.style.format("{:,}")
                            for c in _SLAB_COLS + ["Total"]:
                                if c in df.columns:
                                    mx = int(df[c].max())
                                    fn = lambda v, cn=c, cm=mx: _slab_cell_colour(v, cn, cm)
                                    try:
                                        s = s.map(fn, subset=[c])
                                    except AttributeError:
                        ...
                        st.dataframe(
                            _style_slab_df(_slab_disp),
                            use_container_width=True,
                            height=min(44 * (len(_slab_disp) + 1) + 38, 480),
                            on_select="rerun",
                            selection_mode="single-cell",
                            key=_slab_tbl_key
                        )

                        # Read selection directly from session_state — this is what persists
                        # across the rerun that on_select triggers. Structure:
                        # st.session_state[key] = {"selection": {"rows": [...], "columns": [...]}}
                        _sel_state = st.session_state.get(_slab_tbl_key, {})
                        _sel_data  = _sel_state.get('selection', {}) if isinstance(_sel_state, dict) else {}
                        _sel_rows  = _sel_data.get('rows', [])
                        _sel_cols  = _sel_data.get('columns', [])

                        # Persist the clicked cell so drilldown stays visible on subsequent reruns
                        if _sel_rows and 0 <= _sel_rows[0] < len(_slab_raw_dates):
                            st.session_state[_slab_click_key] = (
                                _slab_raw_dates[_sel_rows[0]],
                                _sel_cols[0] if (_sel_cols and _sel_cols[0] in _SLAB_COLS) else None
                            )

                        _dd_date_raw  = None
                        _dd_slab_name = None
                        if _slab_click_key in st.session_state:
                            _dd_date_raw, _dd_slab_name = st.session_state[_slab_click_key]

                        if _dd_date_raw is not None:
                            _dd_date_str  = _dd_date_raw.strftime('%Y-%m-%d') if hasattr(_dd_date_raw, 'strftime') else str(_dd_date_raw)
                            _dd_date_disp = pd.to_datetime(_dd_date_str).strftime('%d %b %Y (%a)')
                            _dd_slab_disp = _dd_slab_name if _dd_slab_name else "All Slabs"
                            ...
                            <div style="background:rgba(2,22,14,0.80);border-left:4px solid #059669;
                                ...
                                <span style="font-size:0.82rem;color:#34d399;font-weight:600;">
                                    🔍 Shop Drilldown — {_dd_date_disp} &nbsp;|&nbsp; {_dd_slab_disp}
                                </span>
                            </div>""", unsafe_allow_html=True)

                            _dd_where_parts = [
                                "dt_mod_date IS NOT NULL", "dt_doc_date IS NOT NULL",
                                f"dt_mod_date::DATE = '{_dd_date_str}'"
                            ]
                            if _dd_slab_name and _dd_slab_name in _SLAB_SQL_CONDS:
                                _dd_where_parts.append(_SLAB_SQL_CONDS[_dd_slab_name])
                            _dd_where_sql = " AND ".join(_dd_where_parts)

                            _dd_conn = get_db_connection()
                            if _dd_conn:
                                try:
                                    _dd_df = pd.read_sql(f"""
                                        SELECT
                                    ...
                                    """, _dd_conn)
                                    _dd_conn.close()
                                    if _dd_df is not None and not _dd_df.empty:
                                        st.dataframe(
                                            _dd_df.set_index('Shop').style.format("{:,}").background_gradient(
                                                subset=_SLAB_COLS + ["Total"], cmap='YlOrRd', axis=None
                                            ),
                                            use_container_width=True,
                                            height=min(44 * (len(_dd_df) + 1) + 38, 480)
                                        )
                                    else:
                                    ...
                                    st.warning(f"⚠ Drilldown query error: {_dd_err}")
                        else:
                            st.caption("👆 Click any cell in the table above to see a shop-level breakdown.")
                    else:
                        st.info("No offloading data found for the last 11 days.")

                # Closing Shop zone bar
                st.markdown('<div style="border-bottom:2px solid rgba(16,185,129,0.3);border-radius:0 0 12px 12px;height:6px;background:rgba(4,40,28,0.6);margin-bottom:8px;"></div>', unsafe_allow_html=True)
    ...
    # ==================== OVERALL SUMMARY (7-Day Trends) — after Shop Compliance ====================
    st.markdown("""
        <h3 style='font-size: 1.3rem; font-weight: 700; color: #FFFFFF;
            ...
            📊 Overall Summary — Last 7 Day Trend
        </h3>
    """, unsafe_allow_html=True)
    ...
    with trend_col1:
        ...
        if wh_received_trend is not None and not wh_received_trend.empty:
            fig = px.line(wh_received_trend, x='date', y='uniqueness_pct',
                          title='WH Received Uniqueness %', markers=True)
            fig.update_traces(
            ...
            fig.update_layout(
                height=250, yaxis_title="Uniqueness %", xaxis_title="Date",
                plot_bgcolor='rgba(26,40,71,0.3)', paper_bgcolor='rgba(0,0,0,0)',
                font=dict(family='Inter', size=11, color='#FFFFFF'),
                title_font=dict(size=14, family='Poppins', color='#FFFFFF', weight=600),
                hovermode='x unified',
                hoverlabel=dict(bgcolor='rgba(26,40,71,0.95)', font_size=12, font_family='Inter', font_color='white'),
            ...
            st.plotly_chart(fig, use_container_width=True, key="wh_received_trend_final")
        else:
            st.info("No trend data available.")
    with trend_col2:
        wh_loaded_trend = get_wh_uniqueness_trend('loaded')
        if wh_loaded_trend is not None and not wh_loaded_trend.empty:
            fig = px.line(wh_loaded_trend, x='date', y='uniqueness_pct',
                          title='WH Loaded Uniqueness %', markers=True)
            fig.update_traces(
                line=dict(color='#FFB84D', width=3, shape='spline'),
                marker=dict(size=10, color='#FFB84D', symbol='square',
                            line=dict(color='rgba(255,255,255,0.8)', width=2)),
                hovertemplate='<b>%{x}</b><br>Uniqueness: %{y:.1f}%<extra></extra>'
            )
            _yday_f = datetime.today().date() - timedelta(days=1)
            _last7_f = pd.date_range(_yday_f - timedelta(days=6), _yday_f).strftime('%Y-%m-%d').tolist()
            fig.update_layout(
                height=250, yaxis_title="Uniqueness %", xaxis_title="Date",
                xaxis=dict(tickformat="%Y-%m-%d", tickmode="array", tickvals=_last7_f,
                           gridcolor='rgba(91,84,255,0.1)', showgrid=True),
                yaxis=dict(autorange=True, gridcolor='rgba(91,84,255,0.1)', showgrid=True),
                plot_bgcolor='rgba(26,40,71,0.3)', paper_bgcolor='rgba(0,0,0,0)',
                font=dict(family='Inter', size=11, color='#FFFFFF'),
                title_font=dict(size=14, family='Poppins', color='#FFFFFF', weight=600),
            ...
            st.plotly_chart(fig, use_container_width=True, key="wh_loaded_trend_final")
        else:
            st.info("No trend data available.")
    with trend_col3:
        _today_f = datetime.today().date()
        _yday_f2 = _today_f - timedelta(days=1)
        _cstart_f = _yday_f2 - timedelta(days=6)
        _conn_f = get_db_connection()
        if _conn_f:
            try:
                _df_f = pd.read_sql(f"""
                    SELECT DATE(bill_date) as date,
                        ...
                        CASE WHEN COUNT(*)>0 THEN ROUND(COUNT(*) FILTER (WHERE TRIM(serial_check)='Y')::NUMERIC/COUNT(*)*100,2) ELSE 0 END as compliance_pct
                    FROM serialno_check_yes_no
                    WHERE DATE(bill_date) >= '{_cstart_f}' AND DATE(bill_date) <= '{_yday_f2}'
                    GROUP BY DATE(bill_date) ORDER BY DATE(bill_date)
                """, _conn_f)
                _conn_f.close()
                if _df_f is not None and not _df_f.empty:
                    _df_f['date'] = pd.to_datetime(_df_f['date']).dt.strftime('%d %b')
                    fig = px.line(_df_f, x='date', y='compliance_pct',
                                  title='Compliance % (Last 7 Days)', markers=True)
                    fig.update_traces(
                        line=dict(color="#00D97E", width=4, shape='spline'),
                        marker=dict(size=14, color='#00D97E', symbol='circle',
                                    line=dict(color='rgba(255,255,255,0.9)', width=3)),
                        text=_df_f['compliance_pct'].apply(lambda x: f'{x:.1f}%'),
                        textposition="top center",
                        textfont=dict(size=13, family='Poppins', color='#FFFFFF', weight=600),
                        hovertemplate='<b>%{x}</b><br>Compliance: %{y:.1f}%<extra></extra>'
                    )
                    fig.update_layout(
                        height=250, yaxis_title="Compliance %", xaxis_title="Date",
                        margin=dict(l=20, r=20, t=40, b=20),
                        plot_bgcolor='rgba(26,40,71,0.3)', paper_bgcolor='rgba(0,0,0,0)',
                        font=dict(family='Inter', size=11, color='#FFFFFF'),
                        title_font=dict(size=14, family='Poppins', color='#FFFFFF', weight=600),
                        hovermode='x unified',
                        hoverlabel=dict(bgcolor='rgba(26,40,71,0.95)', font_size=13, font_family='Inter', font_color='white'),
                        xaxis=dict(gridcolor='rgba(91,84,255,0.1)', showgrid=True,
                                   tickfont=dict(size=13, color='#FFFFFF')),
                        yaxis=dict(autorange=True, gridcolor='rgba(91,84,255,0.1)', showgrid=True,
                                   tickfont=dict(size=13, color='#FFFFFF'))
                    )
                    st.plotly_chart(fig, use_container_width=True, key="compliance_7day_final")
                else:
                    st.info("No compliance data available.")
            except Exception as e:
                st.error(f"Error loading compliance trend: {e}")
                if _conn_f: _conn_f.close()
        else:
            st.info("No compliance data available.")
        ...
        if shop_metrics:
            ...
            with col5:
                ...
                <div class="metric-card {card_class}" title="Shop Sold Serial Compliance %">
                    ...
                    <div class="metric-change">Compliance: Unique Sold / Total Sold with Serial No</div>
                </div>
                """, unsafe_allow_html=True)
    ...
    if False:
        ...
        if sales_metrics and dup_sales is not None:
            ...
            with col3:
                st.markdown(f"""
                <div class="metric-card danger" title="Total Sold with Serial No minus Unique Sold">
    ...
    if False:
        ...
        wh_metrics = get_warehouse_metrics(warehouse_range, warehouse_range, shop_range=warehouse_range)
